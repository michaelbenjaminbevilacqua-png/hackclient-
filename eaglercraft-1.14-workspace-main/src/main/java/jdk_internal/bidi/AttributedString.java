...
public class AttributedString {
     ...
     *
     * @param iterators AttributedCharacterIterators to construct AttributedString
     *                  from.
     ...
     * @throws NullPointerException     if {@code text} or {@code attributes} is
     *                                  null.
     * @throws IllegalArgumentException if the text has length 0 and the attributes
     *                                  parameter is not an empty Map (attributes
     ...
     * range produces an empty text, all attributes will be discarded. Note that any
     * attributes wrapped by an Annotation object are discarded for a subrange of
     * the original attribute range.
     *
     * @param text       The text for this attributed string.
     * @param beginIndex Index of the first character of the range.
     * @param endIndex   Index of the character following the last character of the
     *                   range.
     * @throws NullPointerException     if {@code text} is null.
     * @throws IllegalArgumentException if the subrange given by beginIndex and
     *                                  endIndex is out of the text range.
     ...
     * Note that any attributes wrapped by an Annotation object are discarded for a
     ...
     * @param endIndex   Index of the character following the last character of the
     *                   range.
     * @param attributes Specifies attributes to be extracted from the text. If null
     *                   is specified, all available attributes will be used.
     * @throws NullPointerException     if {@code text} is null.
     * @throws IllegalArgumentException if the subrange given by beginIndex and
     *                                  endIndex is out of the text range.
     * @see java.text.Annotation
     ...
     *                                  (attributes cannot be applied to a 0-length
     *                                  range).
     */
     ...
     *                                  greater than the length of the string, or
     ...
     *                                  beginIndex and endIndex together don't
     *                                  define a non-empty subrange of the string
     *                                  and the attributes parameter is not an empty
     *                                  Map.
     */
     ...
     * <ul>
     * <li>If copyAttrs is true, the attributes from the existing run will be placed
     * in both of the newly created runs.
    ...
    private final int ensureRunBreak(int offset, boolean copyAttrs) {
        ...
        if (runCount == currentCapacity) {
            ...
            // use temporary variables so things remain consistent in case of an exception
            int[] newRunStarts = Arrays.copyOf(runStarts, newCapacity);
            Vector<Attribute>[] newRunAttributes = Arrays.copyOf(runAttributes, newCapacity);
            Vector<Object>[] newRunAttributeValues = Arrays.copyOf(runAttributeValues, newCapacity);
        ...
        // make copies of the attribute information of the old run that the new one used
        // to be part of
        // use temporary variables so things remain consistent in case of an exception
    ...
    // add the attribute attribute/value to all runs where beginRunIndex <= runIndex
    // < endRunIndex
    ...
    private Object getAttributeCheckRange(Attribute attribute, int runIndex, int beginIndex, int endIndex) {
        ...
        if (value instanceof Annotation) {
            // need to check whether the annotation's range extends outside the iterator's
            // range
            if (beginIndex > 0) {
    ...
    /**
     * Appends the contents of the CharacterIterator iterator into the StringBuffer
     * buf.
     */
     ...
     * (typically the end of the text) to the ones specified in attrs. This is only
     * meant to be called from the constructor!
     */
    ...
    private final class AttributedStringIterator implements AttributedCharacterIterator {
        ...
        // we don't synchronize on the iterator, assuming that an iterator is only used
        // in one thread.
