...
import jdk_internal.bidi.AttributedCharacterIterator;
 ...
 * <p>
 * The "limit" of a sequence of characters is the position just after their last
 * character, i.e., one more than that position.
 * <p>
 * <p>
 * Some of the API methods provide access to "runs". Such a "run" is defined as
 * a sequence of characters that are at the same embedding level after
 * performing the Bidi algorithm.
 *
 * <h3>Basic concept: paragraph</h3> A piece of text can be divided into several
 * paragraphs by characters with the Bidi class <code>Block Separator</code>.
 ...
 * </ul>
 *
 * <h3>Basic concept: text direction</h3> The direction of a piece of text may
 * be:
 * <ul>
 * <li>{@link #LTR}
 * <li>{@link #RTL}
 ...
 * <p>
 * <p>
 * Levels can be abstract values when used for the <code>paraLevel</code> and
 * <code>embeddingLevels</code> arguments of <code>setPara()</code>; there:
 * <ul>
 * <li>the high-order bit of an <code>embeddingLevels[]</code> value indicates
 ...
 * <li><code>paraLevel</code> can be set to the pseudo-level values
 * <code>LEVEL_DEFAULT_LTR</code> and <code>LEVEL_DEFAULT_RTL</code>.</li>
 * </ul>
 *
 * <p>
 * The related constants are not real, valid level values.
 * <code>DEFAULT_XXX</code> can be used to specify a default for the paragraph
 * level for when the <code>setPara()</code> method shall determine it but there
 * is no strongly typed character in the input.
 * <p>
 * <p>
 * Note that the value for <code>LEVEL_DEFAULT_LTR</code> is even and the one
 * for <code>LEVEL_DEFAULT_RTL</code> is odd, just like with normal LTR and RTL
 ...
 * </ul>
 *
 * <h3>Basic concept: Reordering Mode</h3> Reordering mode values indicate which
 ...
 * </ul>
 *
 * <h3>Basic concept: Reordering Options</h3> Reordering options can be applied
 * during Bidi text transformations.
 *
 * <p>
 * <b>See Also:</b>
 * <ul>
 ...
 * </ul>
 *
 * @author Simon Montagu, Matitiahu Allouche (ported from C code written by
 * Markus W. Scherer)
 * @stable ICU 3.8
 *
 *
 ...
 * <p>
 * This is (hypothetical) sample code that illustrates how the ICU Bidi
 * API could be used to render a paragraph of text. Rendering code
 * depends highly on the graphics system, therefore this sample code
 * must make a lot of assumptions, which may or may not match any
 * existing graphics system's properties.
 *
 * <p>
 ...
 *         <li>A run of single-style, unidirectional text can be rendered at
 *         once.</li>
 ...
 *         Locale-dependent - and therefore its implementation omitted from this
 *         sample code.</li>
 *         </ul>
 *
 *         <pre>{@code
...
public class BidiBase {
     ...
     * <code>REORDER_INVERSE_FOR_NUMBERS_SPECIAL</code>, the text to reorder is
     ...
     * <p>
     * <p>
     * If reordering option <code>OPTION_INSERT_MARKS</code> is set, an RLM may be
     ...
     * <code>MAX_EXPLICIT_LEVEL+1</code>).
     *
     * @stable ICU 3.8
     */
     ...
     * <code>OPTION_REMOVE_CONTROLS</code> is specified. This can also happen for
     ...
     * Left-to-right text.
     * <ul>
     * <li>As return value for <code>getDirection()</code>, it means that the source
     * string contains no right-to-left characters, or that the source string is
     * empty and the paragraph level is even.
     * <li>As return value for <code>getBaseDirection()</code>, it means that the
     ...
     * <p>
     * As return value for <code>getDirection()</code>, it means that the source
    ...
    /**
     * option bit for writeReordered(): remove Bidi control characters (this does
     * not affect INSERT_LRM_FOR_NUMERIC)
     *
     * <p>
     * This option does not imply corresponding adjustment of the index mappings.
     * </p>
     ...
     *
     * <p>
     * This has the same effect as calling <code>writeReordered()</code> first
     * without this option, and then calling <code>writeReverse()</code> without
    ...
    /**
     * Reordering mode: Logical to Visual algorithm which handles numbers in a way
     * which mimicks the behavior of Windows XP.
     *
     * @stable ICU 3.8
     * @see #setReorderingMode
     */
    ...
    /**
     * Reordering mode: Logical to Visual algorithm grouping numbers with adjacent R
    ...
    /**
     * Reordering mode: Reorder runs only to transform a Logical LTR string to the
     * logical RTL string with the same display, or vice-versa.<br>
     * If this mode is set together with option <code>OPTION_INSERT_MARKS</code>,
     ...
     * (same algorithm as selected by <code>setInverse(true)</code>.
     *
     * @stable ICU 3.8
     * @see #setInverse
     * @see #setReorderingMode
    ...
    /**
     * Option bit for <code>setReorderingOptions</code>: insert Bidi marks (LRM or
     * RLM) when needed to ensure correct result of a reordering to a Logical order
     *
     * <p>
     * This option must be set or reset before calling <code>setPara</code>.
     * </p>
     *
     * <p>
     * This option is significant only with reordering modes which generate a result
     * with Logical order, specifically.
     * </p>
     * <ul>
     ...
     * removal of Bidi marks (option <code>OPTION_REMOVE_CONTROLS</code> set before
     * calling <code>setPara()</code> or option <code>REMOVE_BIDI_CONTROLS</code> in
     * <code>writeReordered</code>), the result will be identical to the source text
    ...
    /**
     * Option bit for <code>setReorderingOptions</code>: remove Bidi control
     * characters
     *
     * <p>
     * This option must be set or reset before calling <code>setPara</code>.
     ...
     *
     * <p>
     * This option nullifies option <code>OPTION_INSERT_MARKS</code>. It inhibits
     * option <code>INSERT_LRM_FOR_NUMERIC</code> in calls to method
     * <code>writeReordered()</code> and it implies option
    ...
    /**
     * Option bit for <code>setReorderingOptions</code>: process the output as part
     ...
     *
     * <p>
     * This option specifies that the caller is interested in processing large text
     * object in parts. The results of the successive calls are expected to be
     * concatenated by the caller. Only the call for the last part will have this
     * option bit off.
     * </p>
     *
     * <p>
     * When this option bit is on, <code>setPara()</code> may process less than the
     * full source text in order to truncate the text at a meaningful boundary. The
     * caller should call <code>getProcessedLength()</code> immediately after
     * calling <code>setPara()</code> in order to determine how much of the source
     * text has been processed. Source text beyond that length should be resubmitted
     * in following calls to <code>setPara</code>. The processed length may be less
     ...
     *
     * <p>
     * When the <code>OPTION_STREAMING</code> option is used, it is recommended to
     * call <code>orderParagraphsLTR(true)</code> before calling
    ...
    /*
     * Comparing the description of the Bidi algorithm with this implementation is
     * easier with the same names for the Bidi types in the code as there. See
     * UCharacterDirection
     */
     ...
     * <a href="http://www.unicode.org/reports/tr9/">Unicode Standard Annex #9:
     * Unicode Bidirectional Algorithm</a>. Returns UCharacter.BidiPairedBracketType
     * values.
     *
     * @stable ICU 52
     */
    ...
    /*
     * The following bit is used for the directional isolate status. Stack entries
     * corresponding to isolate sequences are greater than ISOLATE.
     */
    static final int ISOLATE = 0x0100;

    /*
     * reference to parent paragraph object (reference to self if this object is a
     * paragraph object); set to null in a newly opened object; set to a real value
     * after a successful execution of setPara or setLine
     */
    BidiBase paraBidi;
    ...
    /*
     * if the option OPTION_STREAMING is set, this is the length of text actually
     * processed by <code>setPara</code>, which may be shorter than the original
    ...
    /**
     * Allocate a <code>Bidi</code> object with preallocated memory for internal
     * structures. This method provides a <code>Bidi</code> object like the default
     ...
     * some values to 0 here. That means that if, e.g., <code>maxRunCount</code>
     * cannot be reasonably predetermined and should not be set to
     * <code>maxLength</code> (the only failproof value) to avoid wasting memory,
     * then <code>maxRunCount</code> could be set to 0 here and the internal
     ...
     *                    anywhere between 1 and <code>maxLength</code>. It is
     *                    typically small.
    ...
    private void resolveImplicitLevels(int start, int limit, short sor, short eor) {
        ...
        for (i = start; i <= limit; i++) {
            if (i >= limit) {
                int k;
                for (k = limit - 1; k > start && (DirPropFlag(dirProps[k]) & MASK_BN_EXPLICIT) != 0; k--)
                    ;
                dirProp = dirProps[k];
                if (dirProp == LRI || dirProp == RLI)
