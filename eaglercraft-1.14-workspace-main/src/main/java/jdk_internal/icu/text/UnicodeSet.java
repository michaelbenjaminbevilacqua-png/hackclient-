 ...
 * <p>
 * Note: method freeze() will not only make the set immutable, but also makes
 * important methods much higher performance: contains(c), containsNone(...),
 * span(...), spanBack(...) etc. After the object is frozen, any subsequent call
 * that wants to change the object will throw UnsupportedOperationException.
 *
 * <p>
 * The UnicodeSet class is not designed to be subclassed.
 *
 * <p>
 * <code>UnicodeSet</code> supports two APIs. The first is the <em>operand</em>
 * API that allows the caller to modify the value of a <code>UnicodeSet</code>
 * object. It conforms to Java 2's <code>java.util.Set</code> interface,
 * although <code>UnicodeSet</code> does not actually implement that interface.
 * All methods of <code>Set</code> are supported, with the modification that
 * they take a character range or single character instead of an
 * <code>Object</code>, and they take a <code>UnicodeSet</code> instead of a
 * <code>Collection</code>. The operand API may be thought of in terms of
 * boolean logic: a boolean OR is implemented by <code>add</code>, a boolean AND
 * is implemented by <code>retain</code>, a boolean XOR is implemented by
 * <code>complement</code> taking an argument, and a boolean NOT is implemented
 * by <code>complement</code> with no argument. In terms of traditional set
 * theory function names, <code>add</code> is a union, <code>retain</code> is an
 * intersection, <code>remove</code> is an asymmetric difference, and
 * <code>complement</code> with no argument is a set complement with respect to
 * the superset range <code>MIN_VALUE-MAX_VALUE</code>
 *
 * <p>
 * The second API is the <code>applyPattern()</code>/<code>toPattern()</code>
 * API from the <code>java.text.Format</code>-derived classes. Unlike the
 * methods that add characters, add categories, and control the logic of the
 * set, the method <code>applyPattern()</code> sets all attributes of a
 ...
 * </p>
 * <p>
 * Patterns are accepted by the constructors and the <code>applyPattern()</code>
 * methods and returned by the <code>toPattern()</code> method. These patterns
 ...
 * meaning. White space characters, as defined by the Unicode
 ...
 * property patterns, see the User's Guide for UnicodeSet at
 ...
 * precedence and bind left-to-right. Thus "[[:L:]-[a-z]-[\\u0100-\\u01FF]]" is
 * equivalent to "[[[:L:]-[a-z]]-[\\u0100-\\u01FF]]". This only really matters
 * for difference; intersection is commutative.
 *
 * <table>
 * <tr valign=top>
 ...
 * align="right"><code>pattern-expr :=&nbsp; </code></td>
 ...
 * </em><code>Character.digit(c, 16)</code><em> returns a non-negative
 * result</em></td>
 * </tr>
 * <tr>
 ...
 * <ul>
 * <li>{@link #ranges()} to iterate through the ranges</li>
 * <li>{@link #strings()} to iterate through the strings</li>
 * <li>{@link #iterator()} to iterate through the entire contents in a single
 * loop. That method is, however, not particularly efficient, since it "boxes"
 * each code point into a String.
 * </ul>
 * All of the above can be used in <b>for</b> loops. The
 * {@link com.ibm.icu.text.UnicodeSetIterator UnicodeSetIterator} can also be
 * used, but not in <b>for</b> loops.
 * <p>
...
public class UnicodeSet {
    ...
    /**
     * The pattern representation of this set. This may not be the most economical
     * pattern. It is the pattern supplied to applyPattern(), with variables
     * substituted and whitespace removed. For sets constructed without
     * applyPattern(), or modified using the non-pattern API, this string will be
     * null, indicating that toPattern() must generate a pattern representation from
     * the inversion list.
     */
     ...
     * @throws IllegalArgumentException if the pattern contains a
     *                                  syntax error.
     * @stable ICU 2.0
     */
     ...
     *
     * @param other a <code>UnicodeSet</code> whose value will be copied to this
     *              object
     * @stable ICU 2.0
     */
    ...
    /**
     * Returns the number of elements in this set (its cardinality) Note than the
     * elements of a set may include both individual codepoints and strings.
     *
     ...
     * this set already contains the specified character, the call leaves this set
     * unchanged.
     *
     * @stable ICU 2.0
     */
     ...
     * If this set already contains the multicharacter, the call leaves this set
     * unchanged. Thus {@code "ch" => {"ch"}} <br>
    ...
    /**
     * Same as span() but also counts the smallest number of set elements on any
     * path across the span.
     * <p>
     * To replace, count elements, or delete spans, see
     * {@link com.ibm.icu.text.UnicodeSetSpanner UnicodeSetSpanner}.
     *
    ...
    /**
     * Span a string backwards (from the fromIndex) using this UnicodeSet. If the
     * fromIndex is less than 0, spanBack will return 0. If fromIndex is greater
     * than the string length, spanBack will start from the string length.
     * <p>
     * To replace, count elements, or delete spans, see
     * {@link com.ibm.icu.text.UnicodeSetSpanner UnicodeSetSpanner}.
     *
    ...
    /**
     * Argument values for whether span() and similar functions continue while the
     * current character is contained vs. not contained in the set.
     * <p>
     ...
     * <li>span() and spanBack() partition any string the same way when alternating
     * between span(NOT_CONTAINED) and span(either "contained" condition).
     * <li>Using a complemented (inverted) set and the opposite span conditions
     ...
     * strings:
     * <ul>
     * <li>The complement of the set contains the opposite set of code points, but
     * the same set of strings. Therefore, complementing both the set and the span
     * conditions may yield different results.
     * <li>When starting spans at different positions in a string (span(s, ...) vs.
     * span(s+1, ...)) the ends of the spans may be different because a set string
     * may start before the later position.
     * <li>span(SIMPLE) may be shorter than span(CONTAINED) because it will not
     * recursively try all possible paths. For example, with a set which contains
     * the three strings "xy", "xya" and "ax", span("xyax", CONTAINED) will return 4
     * but span("xyax", SIMPLE) will return 3. span(SIMPLE) will never be longer
     * than span(CONTAINED).
     * <li>With either "contained" condition, span() and spanBack() may partition a
     * string in different ways. For example, with a set which contains the two
     * strings "ab" and "ba", and when processing the string "aba", span() will
     * yield contained/not-contained boundaries of { 0, 2, 3 } while spanBack() will
     * yield boundaries of { 0, 1, 3 }.
     * </ul>
    ...
    public enum SpanCondition {
         ...
         * <code>(OR of each set element)*</code>. (Java/ICU/Perl regex stops at the
