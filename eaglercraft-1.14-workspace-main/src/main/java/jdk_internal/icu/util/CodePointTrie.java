 ...
 * code points (U+0000..U+10FFFF) to integer values. For details see
...
public abstract class CodePointTrie extends CodePointMap {
    /**
     * Selectors for the type of a CodePointTrie. Different trade-offs for size vs.
     * speed.
     *
     * <p>
     * Use null for {@link #fromBinary} to accept any type; {@link #getType} will
     * return the actual type.
     *
     * @stable ICU 63
     * @see MutableCodePointTrie#buildImmutable(Type,
     * ValueWidth)
     * @see #fromBinary
     * @see #getType
     */
    public enum Type {
        /**
         * Fast/simple/larger BMP data structure. The {@link Fast} subclasses have
         * additional functions for lookup for BMP and supplementary code points.
         *
         * @stable ICU 63
         * @see Fast
         */
    ...
    public enum ValueWidth {
        ...
        BITS_32,
        /**
         * The trie stores 8 bits per data value. It returns them as unsigned values
         * 0..0xff=255.
         *
         * @stable ICU 63
         */
    ...
    /**
     * Creates a trie from its binary form, stored in the ByteBuffer starting at the
     * current position. Advances the buffer position to just after the trie data.
     * Inverse of {@link #toBinary(OutputStream)}.
     *
     * <p>
     * The data is copied from the buffer; later modification of the buffer will not
     * affect the trie.
     *
     * @param type       selects the trie type; this method throws an exception if
     *                   the type does not match the binary data; use null to accept
     *                   any type
     * @param valueWidth selects the number of bits in a data value; this method
     *                   throws an exception if the valueWidth does not match the
     *                   binary data; use null to accept any data value width
    ...
    public static CodePointTrie fromBinary(Type type, ValueWidth valueWidth, ByteBuffer bytes) {
        ...
        try {
            ...
            /**
             * Options bit field: Bits 15..12: Data length bits 19..16. Bits 11..8: Data
             * null block offset bits 19..16. Bits 7..6: UCPTrieType Bits 5..3: Reserved
             * (0). Bits 2..0: UCPTrieValueWidth
             */
             ...
             * and then shifted right by SHIFT_2.
             */
            ...
            switch (valueWidth) {
                case BITS_16: {
                    ...
                    return type == Type.FAST ? new Fast16(index, data16, highStart, index3NullOffset, dataNullOffset)
                            : new Small16(index, data16, highStart, index3NullOffset, dataNullOffset);
                }
                case BITS_32: {
                    int[] data32 = ICUBinary.getInts(bytes, dataLength, 0);
                    return type == Type.FAST ? new Fast32(index, data32, highStart, index3NullOffset, dataNullOffset)
                            : new Small32(index, data32, highStart, index3NullOffset, dataNullOffset);
                }
                case BITS_8: {
                    byte[] data8 = ICUBinary.getBytes(bytes, dataLength, 0);
                    return type == Type.FAST ? new Fast8(index, data8, highStart, index3NullOffset, dataNullOffset)
    ...
    private static final int maybeFilterValue(int value, int trieNullValue, int nullValue, ValueFilter filter) {
                    ...
                    if (block == dataNullOffset) {
                        ...
                        if (haveValue) {
                            if (trieValue2 != trieValue) {
                                if (filter == null
                                        || maybeFilterValue(trieValue2, this.nullValue, nullValue, filter) != value) {
                                    range.set(start, c - 1, value);
                                    return true;
                                }
    ...
    /**
     * Writes a representation of the trie to the output stream. Inverse of
     * {@link #fromBinary}.
     *
    ...
    public final int toBinary(OutputStream os) {
        try {
            ...
            dos.writeChar( // options
                    ((dataLength & 0xf0000) >> 4) | ((dataNullOffset & 0xf0000) >> 8) | (getType().ordinal() << 6)
    ...
    /**
     * Offset from dataLength (to be subtracted) for fetching the value returned for
     * out-of-range code points and ill-formed UTF-8/16.
     *
     * @internal
     */
    ...
    /**
     * Offset from dataLength (to be subtracted) for fetching the value returned for
     * code points highStart..U+10FFFF.
     *
     * @internal
     */
    ...
    /**
     * Difference between two shift sizes, for getting an index-2 offset from an
     * index-3 offset. 5=9-4
     */
    static final int SHIFT_2_3 = SHIFT_2 - SHIFT_3;

    /**
     * Difference between two shift sizes, for getting an index-1 offset from an
     * index-2 offset. 5=14-9
     */
    static final int SHIFT_1_2 = SHIFT_1 - SHIFT_2;

    /**
     * Number of index-1 entries for the BMP. (4) This part of the index-1 table is
    ...
    /**
     * Value for index3NullOffset which indicates that there is no index-3 null
     * block. Bit 15 is unused for this value because this bit is used if the
     * index-3 contains 18-bit indexes.
     */
    ...
    /**
     * Internal index-3 null block offset. Set to an impossibly high value (e.g.,
     * 0xffff) if there is no dedicated index-3 null block.
     *
     * @internal
     */
    private final int index3NullOffset;
    /**
     * Internal data null block offset, not shifted. Set to an impossibly high value
     * (e.g., 0xfffff) if there is no dedicated data null block.
     *
     * @internal
     */
    ...
    public static abstract class Fast extends CodePointTrie {
        ...
        /**
         * Creates a trie from its binary form. Same as
         * {@link CodePointTrie#fromBinary(Type, ValueWidth, ByteBuffer)} with
         * {@link Type#FAST}.
         *
         * @param valueWidth selects the number of bits in a data value; this method
         *                   throws an exception if the valueWidth does not match the
    ...
    public static final class Fast16 extends Fast {
         ...
         * Creates a trie from its binary form. Same as
         * {@link CodePointTrie#fromBinary(Type, ValueWidth, ByteBuffer)} with
         * {@link Type#FAST} and {@link ValueWidth#BITS_16}.
         *
         * @param bytes a buffer containing the binary data of a CodePointTrie
         * @return the trie
         * @stable ICU 63
    ...
    public static final class Fast32 extends Fast {
         ...
         * Creates a trie from its binary form. Same as
         * {@link CodePointTrie#fromBinary(Type, ValueWidth, ByteBuffer)} with
         * {@link Type#FAST} and {@link ValueWidth#BITS_32}.
         *
         * @param bytes a buffer containing the binary data of a CodePointTrie
         * @return the trie
         * @stable ICU 63
    ...
    public static final class Fast8 extends Fast {
         ...
         * Creates a trie from its binary form. Same as
         * {@link CodePointTrie#fromBinary(Type, ValueWidth, ByteBuffer)} with
         * {@link Type#FAST} and {@link ValueWidth#BITS_8}.
         *
         * @param bytes a buffer containing the binary data of a CodePointTrie
         * @return the trie
         * @stable ICU 63
    ...
    public static final class Small16 extends Small {
         ...
         * Creates a trie from its binary form. Same as
         * {@link CodePointTrie#fromBinary(Type, ValueWidth, ByteBuffer)} with
         * {@link Type#SMALL} and {@link ValueWidth#BITS_16}.
         *
         * @param bytes a buffer containing the binary data of a CodePointTrie
         * @return the trie
         * @stable ICU 63
    ...
    public static final class Small32 extends Small {
         ...
         * Creates a trie from its binary form. Same as
         * {@link CodePointTrie#fromBinary(Type, ValueWidth, ByteBuffer)} with
         * {@link Type#SMALL} and {@link ValueWidth#BITS_32}.
         *
         * @param bytes a buffer containing the binary data of a CodePointTrie
         * @return the trie
         * @stable ICU 63
    ...
    public static final class Small8 extends Small {
         ...
         * Creates a trie from its binary form. Same as
         * {@link CodePointTrie#fromBinary(Type, ValueWidth, ByteBuffer)} with
         * {@link Type#SMALL} and {@link ValueWidth#BITS_8}.
         *
         * @param bytes a buffer containing the binary data of a CodePointTrie
         * @return the trie
         * @stable ICU 63
