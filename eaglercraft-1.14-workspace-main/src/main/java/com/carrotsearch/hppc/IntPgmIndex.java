...
/**
 * Space-efficient index that enables fast rank/range search operations on a
 * sorted sequence of <code>int</code>.
 *
 * <p>
 * Implementation of the PGM-Index described at
 * <a href="https://pgm.di.unipi.it/">https://pgm.di.unipi.it/</a>, based on the
 * paper
 *
 * <pre>
 ...
 * {@code contains()} is between 4x to 7x slower than
 * {@code IntHashSet#contains()}, but between 2.5x to 3x faster than
 * {@link Arrays#binarySearch}.
 *
 * <p>
 * Its compactness (40KB for 200MB of keys) makes it efficient for very large
 * collections, the index fitting easily in the L2 cache. The {@code epsilon}
 * parameter should be set according to the desired space-time trade-off. A
 * smaller value makes the estimation more precise and the range smaller but at
 * the cost of increased space usage. In practice, {@code epsilon} 64 is a good
 * sweet spot.
 *
 * <p>
 * Internally the index uses an optimal piecewise linear mapping from keys to
 * their position in the sorted order. This mapping is represented as a sequence
 * of linear models (segments) which are themselves recursively indexed by other
 * piecewise linear mappings.
 */
@com.carrotsearch.hppc.Generated(date = "2024-06-04T15:20:17+0200", value = "KTypePgmIndex.java")
public class IntPgmIndex implements Accountable {

    /**
     * Empty immutable IntPgmIndex.
     */
    public static final IntPgmIndex EMPTY = new IntEmptyPgmIndex();

    /**
     * Epsilon approximation range when searching the list of keys. Controls the
     * size of the returned search range, strictly greater than 0. It should be set
     * according to the desired space-time trade-off. A smaller value makes the
     * estimation more precise and the range smaller but at the cost of increased
     * space usage.
     *
     * <p>
     * With EPSILON=64 the benchmark with 200MB of keys shows that this PGM index
     * requires only 2% additional memory on average (40KB). It depends on the
     * distribution of the keys. This epsilon value is good even for 2MB of keys.
     * With EPSILON=32: +5% speed, but 4x space (160KB).
     */
    public static final int EPSILON = 64;

    /**
     * Epsilon approximation range for the segments layers. Controls the size of the
     * search range in the hierarchical segment lists, strictly greater than 0.
     */
    public static final int EPSILON_RECURSIVE = 32;

    /**
     * Size of a key, measured in {@link Integer#BYTES} because the key is stored in
     * an int[].
     */
    public static final int KEY_SIZE = RamUsageEstimator.primitiveSizes.get(int.class) / Integer.BYTES;
    ...
    /**
     * Data size of a segment, measured in {@link Integer#BYTES}, because segments
     * are stored in an int[].
     */
    ...
    /**
     * The list of keys for which this index is built. It is sorted and may contain
     * duplicate elements.
     */
    public final IntArrayList keys;

    /**
     * The size of the key set. That is, the number of distinct elements in
     * {@link #keys}.
     */
    ...
    /**
     * The offsets in {@link #segmentData} of the first segment of each segment
     * level.
     */
    ...
    private IntPgmIndex(IntArrayList keys, int size, int epsilon, int epsilonRecursive, int[] levelOffsets,
                        int[] segmentData) {
        assert keys.size() > 0;
    ...
    /**
     * Returns the size of the key set. That is, the number of distinct elements in
     * {@link #keys}.
     */
    public int size() {
        return size;
    ...
    /**
     * Searches the specified key, and returns its index in the element list. If
     * multiple elements are equal to the specified key, there is no guarantee which
     * one will be found.
     *
     * @return The index of the searched key if it is present; otherwise,
     * {@code (-(<i>insertion
     * point</i>) - 1)}. The <i>insertion point</i> is defined as the point at
     * which the key would be inserted into the list: the index of the first
     * element greater than the key, or {@link #keys}#{@code size()} if all
     * the elements are less than the specified key. Note that this
    ...
    public int indexOf(int key) {
        ...
        int index = Math.min(approximateIndex(key, segmentDataIndex, segmentData),
    ...
    /**
     * Returns, for any value {@code x}, the number of keys in the sorted list which
     * are smaller than {@code x}. It is equal to {@link #indexOf} if {@code x}
     * belongs to the list, or -{@link #indexOf}-1 otherwise.
     *
     * <p>
     * If multiple elements are equal to the specified key, there is no guarantee
     * which one will be found.
     *
     * @return The index of the searched key if it is present; otherwise, the
     * {@code insertion point}. The <i>insertion point</i> is defined as the
     * point at which the key would be inserted into the list: the index of
     * the first element greater than the key, or {@link #keys}#{@code
     * size()} if all the elements are less than the specified key. Note that
     ...
     * {@code minKey} (inclusive), and less than or equal to {@code maxKey}
     * (inclusive).
     */
    public int rangeCardinality(int minKey, int maxKey) {
    ...
    /**
     * Returns an iterator over the keys in the list that are greater than or equal
     * to {@code minKey} (inclusive), and less than or equal to {@code maxKey}
     * (inclusive).
     */
    public Iterator<IntCursor> rangeIterator(int minKey, int maxKey) {
        int fromIndex = rank(minKey);
        return new RangeIterator(keys, fromIndex, maxKey);
    }

    /**
     * Applies {@code procedure} to the keys in the list that are greater than or
     * equal to {@code
     * minKey} (inclusive), and less than or equal to {@code maxKey} (inclusive).
     */
    public <T extends IntProcedure> T forEachInRange(T procedure, int minKey, int maxKey) {
        final int[] buffer = keys.buffer;
        int k;
    ...
    /**
     * Estimates the allocated memory. It does not count the memory for the list of
     * keys, only for the index itself.
     */
    @Override
    public long ramBytesAllocated() {
        // int: size, epsilon, epsilonRecursive
        // int: firstKey, lastKey
        return RamUsageEstimator.NUM_BYTES_OBJECT_HEADER + 3 * Integer.BYTES + 2L * KEY_SIZE * Integer.BYTES
    ...
    /**
     * Estimates the bytes that are actually used. It does not count the memory for
     * the list of keys, only for the index itself.
     */
    @Override
    ...
    /**
     * Finds the segment responsible for a given key, that is, the rightmost segment
     * having its first key <= the searched key.
     *
    ...
    private int findSegment(int key) {
        ...
        while (--level >= 0) {
            ...
            if (getKey(sdIndex, segmentData) <= key) {
                // Scan sequentially segments after the approximated index, within the epsilon
                // range.
                ...
                // Scan sequentially segments before the approximated index, within the epsilon
                // range.
    ...
    private int approximateIndex(int key, int segmentDataIndex, int[] segmentData) {
        long intercept = getIntercept(segmentDataIndex, segmentData);
        int sKey = getKey(segmentDataIndex, segmentData);
        double slope = getSlope(segmentDataIndex, segmentData);
        int index = (int) (slope * ((double) key - sKey) + intercept);
        return Math.max(index, 0);
    }

    private int getKey(int segmentDataIndex, int[] segmentData) {
        return PgmIndexUtil.getKey(segmentDataIndex + KEY_SIZE, segmentData, 0);
    }

    /**
     * Empty immutable PGM Index.
     */
    private static class IntEmptyPgmIndex extends IntPgmIndex {

        private final Iterator<IntCursor> emptyIterator = new IntEmptyIterator();
        ...
        private static class IntEmptyIterator extends AbstractIterator<IntCursor> {
            @Override
     ...
     */
    protected static class RangeIterator extends AbstractIterator<IntCursor> {
        ...
        /**
         * Range iterator from {@code fromIndex} (inclusive) to {@code maxKey}
         * (inclusive).
         */
        protected RangeIterator(IntArrayList keys, int fromIndex, int maxKey) {
    ...
    /**
     * Builds a {@link IntPgmIndex} on a provided sorted list of keys.
     */
    public static class IntBuilder implements PlaModel.SegmentConsumer, Accountable {
        ...
        /**
         * Sets the sorted list of keys to build the index for; duplicate elements are
         * allowed.
         */
        public IntBuilder setSortedKeys(IntArrayList keys) {
            this.keys = keys;
            return this;
        }

        /**
         * Sets the sorted array of keys to build the index for; duplicate elements are
         * allowed.
         */
        public IntBuilder setSortedKeys(int[] keys, int length) {
            IntArrayList keyList = new IntArrayList(0);
            keyList.buffer = keys;
        ...
        /**
         * Sets the epsilon range to use when learning the segments for the list of
         * keys.
         */
        public IntBuilder setEpsilon(int epsilon) {
            if (epsilon <= 0) {
        ...
        /**
         * Sets the recursive epsilon range to use when learning the segments for the
         * segment levels.
         */
        public IntBuilder setEpsilonRecursive(int epsilonRecursive) {
        ...
        /**
         * Builds the {@link IntPgmIndex}; or {@link #EMPTY} if there are no keys in the
         * list.
         */
        public IntPgmIndex build() {
            if (keys == null || keys.size() == 0) {
                return (IntPgmIndex) EMPTY;
            }
            plam = new PlaModel(epsilon);

            int segmentsInitialCapacity = Math
                    .min(Math.max(keys.size() / (2 * epsilon * epsilon) * SEGMENT_DATA_SIZE, 16), 1 << 19);
            ...
            return new IntPgmIndex(keys, size, epsilon, epsilonRecursive, levelOffsetsFinal, segmentDataFinal);
        }
        ...
        private int buildUpperLevel(int levelOffset, int levelNumSegments) {
            ...
            int key = getKey(segmentDataIndex, segmentData.buffer);
            plam.addKey(key, 0, this);
            for (int i = 1; i < levelNumSegments; i++) {
                segmentDataIndex += SEGMENT_DATA_SIZE;
                int nextKey = getKey(segmentDataIndex, segmentData.buffer);
                if (!((nextKey) == (key))) {
                    key = nextKey;
                    plam.addKey(key, i, this);
                }
        ...
        /**
         * Adds a sentinel segment that is used to give a limit for the position
         * approximation, but does not count in the number of segments per level.
         */
        ...
        public void accept(double firstKey, double slope, long intercept) {
            ...
            PgmIndexUtil.addKey((int) firstKey, segmentData);
        ...
        public long ramBytesAllocated() {
            ...
            return RamUsageEstimator.NUM_BYTES_OBJECT_HEADER + 4 * Integer.BYTES
                    // + keys.ramBytesAllocated()
                    + plam.ramBytesAllocated() + segmentData.ramBytesAllocated();
        }

        /**
         * Estimates the bytes that are actually used. It does not count the memory for
         * the list of keys, only for the builder itself.
         */
        @Override
        public long ramBytesUsed() {
            // int: epsilon, epsilonRecursive, size, numSegments
            return RamUsageEstimator.NUM_BYTES_OBJECT_HEADER + 4 * Integer.BYTES
                    // + keys.ramBytesUsed()
                    + plam.ramBytesUsed() + segmentData.ramBytesUsed();
        }
    }
}
