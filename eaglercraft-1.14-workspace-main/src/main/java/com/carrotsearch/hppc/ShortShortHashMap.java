...
/**
 * A hash map of <code>short</code> to <code>short</code>, implemented using
...
public class ShortShortHashMap implements ShortShortMap, Preallocable, Cloneable, Accountable {
    ...
    private final class EntryIterator extends AbstractIterator<ShortShortCursor> {
        ...
        protected ShortShortCursor fetch() {
            final int mask = ShortShortHashMap.this.mask;
            while (index <= mask) {
                short existing;
                index++;
