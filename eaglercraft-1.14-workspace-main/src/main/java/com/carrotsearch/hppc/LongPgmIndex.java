 ...
 * sorted sequence of <code>long</code>.
 *
 * <p>
...
public class LongPgmIndex implements Accountable {
     ...
     * an int[].
     */
    public static final int KEY_SIZE = RamUsageEstimator.primitiveSizes.get(long.class) / Integer.BYTES;
    ...
    private LongPgmIndex(LongArrayList keys, int size, int epsilon, int epsilonRecursive, int[] levelOffsets,
                         int[] segmentData) {
        assert keys.size() > 0;
    ...
    public static class LongBuilder implements PlaModel.SegmentConsumer, Accountable {
        ...
        /**
         * Builds the {@link LongPgmIndex}; or {@link #EMPTY} if there are no keys in
         * the list.
         */
