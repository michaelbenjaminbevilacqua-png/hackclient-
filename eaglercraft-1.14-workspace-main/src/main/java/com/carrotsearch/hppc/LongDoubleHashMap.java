...
/**
 * A hash map of <code>long</code> to <code>double</code>, implemented using
...
public class LongDoubleHashMap implements LongDoubleMap, Preallocable, Cloneable, Accountable {
    ...
    private final class EntryIterator extends AbstractIterator<LongDoubleCursor> {
        ...
        protected LongDoubleCursor fetch() {
            final int mask = LongDoubleHashMap.this.mask;
            while (index <= mask) {
                long existing;
                index++;
