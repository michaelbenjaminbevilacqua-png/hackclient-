...
/**
 * A hash map of <code>Object</code> to <code>byte</code>, implemented using
...
public class ObjectByteHashMap<KType> implements ObjectByteMap<KType>, Preallocable, Cloneable, Accountable {
    /**
    ...
    private final class EntryIterator extends AbstractIterator<ObjectByteCursor<KType>> {
        ...
        protected ObjectByteCursor<KType> fetch() {
            final int mask = ObjectByteHashMap.this.mask;
            while (index <= mask) {
                KType existing;
                index++;
