...
/**
 * A hash map of <code>Object</code> to <code>short</code>, implemented using
...
public class ObjectShortHashMap<KType> implements ObjectShortMap<KType>, Preallocable, Cloneable, Accountable {
    /**
    ...
    private final class EntryIterator extends AbstractIterator<ObjectShortCursor<KType>> {
        ...
        protected ObjectShortCursor<KType> fetch() {
            final int mask = ObjectShortHashMap.this.mask;
            while (index <= mask) {
                KType existing;
                index++;
