...
/**
 * A hash map of <code>char</code> to <code>double</code>, implemented using
...
public class CharDoubleHashMap implements CharDoubleMap, Preallocable, Cloneable, Accountable {
    ...
    protected boolean equalElements(CharDoubleHashMap other) {
        ...
        for (CharDoubleCursor c : other) {
            char key = c.key;
            if (!containsKey(key) || !(Double.doubleToLongBits(c.value) == Double.doubleToLongBits(get(key)))) {
                return false;
            }
        }
    ...
    private final class EntryIterator extends AbstractIterator<CharDoubleCursor> {
        ...
        protected CharDoubleCursor fetch() {
            final int mask = CharDoubleHashMap.this.mask;
            while (index <= mask) {
                char existing;
                index++;
    ...
    private final class ValuesContainer extends AbstractDoubleCollection {
        ...
        public int removeAll(final double e) {
            return owner.removeAll((key, value) -> (Double.doubleToLongBits(e) == Double.doubleToLongBits(value)));
        }
