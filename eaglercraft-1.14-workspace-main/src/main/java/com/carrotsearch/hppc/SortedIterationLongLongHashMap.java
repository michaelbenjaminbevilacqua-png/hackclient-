 ...
 * entries to another {@link LongLongHashMap} as this may result in a runtime
 * deadlock. See
...
public class SortedIterationLongLongHashMap implements LongLongMap {
     ...
     * values.
     */
    protected int[] sortIterationOrder(int[] entryIndexes, LongLongComparator comparator) {
    ...
    private final class KeysContainer extends AbstractLongCollection implements LongLookupContainer {
        private final SortedIterationLongLongHashMap owner = SortedIterationLongLongHashMap.this;
        ...
        public <T extends LongProcedure> T forEach(final T procedure) {
            owner.forEach((LongLongProcedure) (k, v) -> procedure.apply(k));
            return procedure;
        }

        @Override
        public <T extends LongPredicate> T forEach(final T predicate) {
            owner.forEach((LongLongPredicate) (key, value) -> predicate.apply(key));
            return predicate;
        }
    ...
    private final class ValuesContainer extends AbstractLongCollection {
        ...
        public <T extends LongProcedure> T forEach(T procedure) {
            owner.forEach((LongLongProcedure) (k, v) -> procedure.apply(v));
            return procedure;
        }

        @Override
        public <T extends LongPredicate> T forEach(T predicate) {
            owner.forEach((LongLongPredicate) (k, v) -> predicate.apply(v));
            return predicate;
        }
