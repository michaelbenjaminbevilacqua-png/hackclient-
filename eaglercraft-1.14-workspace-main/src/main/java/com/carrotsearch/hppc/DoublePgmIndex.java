 ...
 * sorted sequence of <code>double</code>.
 *
 * <p>
...
public class DoublePgmIndex implements Accountable {
     ...
     * an int[].
     */
    public static final int KEY_SIZE = RamUsageEstimator.primitiveSizes.get(double.class) / Integer.BYTES;
    ...
    private DoublePgmIndex(DoubleArrayList keys, int size, int epsilon, int epsilonRecursive, int[] levelOffsets,
                           int[] segmentData) {
        assert keys.size() > 0;
    ...
    public static class DoubleBuilder implements PlaModel.SegmentConsumer, Accountable {
        ...
        /**
         * Builds the {@link DoublePgmIndex}; or {@link #EMPTY} if there are no keys in
         * the list.
         */
        public DoublePgmIndex build() {
            ...
            return new DoublePgmIndex(keys, size, epsilon, epsilonRecursive, levelOffsetsFinal, segmentDataFinal);
        }
