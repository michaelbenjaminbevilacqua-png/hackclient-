 ...
 *
 * <p>
 * Learns a mapping that returns a position for a <code>KType</code> key which
 * is at most epsilon away from the correct one in a sorted list of keys. It is
 * optimal and piecewise because it learns the minimum number of
 * epsilon-approximate segments.
 *
 * <p>
 * The PLA-model consists of a sequence segments. A segment s is a triple
 * (key,slope,intercept) that indexes a range of keys through the function fs(k)
 * = k × slope + intercept, which provides an epsilon-approximation of the
 * position of the key k.
 */
public class PlaModel implements Accountable {
    ...
    /**
     * Ordered list of lower points for the current segment. Inside the list,
     * allocated points are re-used.
     */
    private final PointList lower = new PointList(INITIAL_CAPACITY);
    /**
     * Ordered list of upper points for the current segment. Inside the list,
     * allocated points are re-used.
     */
    ...
    /**
     * Previous key used to check that keys are added in strictly increasing
     * sequence.
     */
    ...
    /**
     * Adds a key to this PLA-model. The keys must be provided in a strictly
     * increasing sequence. That is, the key must be greater than the previous key.
     *
     * @param index           The index of the key in the sorted key list.
     * @param segmentConsumer The consumer to call when a new segment is built in
     *                        the PLA-model.
     */
    ...
    /**
     * Finishes the PLA-model construction. Declares that no additional keys will be
     * added. Builds the last segment and calls the provided
     * {@link SegmentConsumer}.
     */
    ...
    public long ramBytesAllocated() {
        ...
        return RamUsageEstimator.NUM_BYTES_OBJECT_HEADER + 4 * Integer.BYTES + 2 * Double.BYTES
                + 6L * Point.RAM_BYTES_ALLOCATED + lower.ramBytesAllocated() + upper.ramBytesAllocated()
    ...
    public interface SegmentConsumer {
        ...
        /**
         * Consumes a new segment. The segment is defined by the epsilon-approximation
         * function fs(k) = k × slope + intercept.
         *
    ...
    private static class Point {
        ...
        static final int RAM_BYTES_ALLOCATED = RamUsageEstimator.NUM_BYTES_OBJECT_HEADER + Double.BYTES + Long.BYTES;
    ...
    /**
     * List of mutable {@link Point}. Re-uses allocated points instead of creating
     * new instances.
     */
    private static class PointList implements Accountable {
        ...
        void add(Point point) {
            if (size == points.length) {
                int newSize = BoundedProportionalArraySizingStrategy.DEFAULT_INSTANCE.grow(points.length, size, 1);
        ...
        public long ramBytesAllocated() {
            ...
            return RamUsageEstimator.NUM_BYTES_OBJECT_HEADER + 2 * Integer.BYTES
                    + RamUsageEstimator.shallowSizeOfArray(points) + (long) numAllocated * Point.RAM_BYTES_ALLOCATED;
        }
