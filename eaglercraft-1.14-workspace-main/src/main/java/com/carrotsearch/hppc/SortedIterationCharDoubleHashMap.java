 ...
 * entries to another {@link CharDoubleHashMap} as this may result in a runtime
 * deadlock. See
...
public class SortedIterationCharDoubleHashMap implements CharDoubleMap {
     ...
     * values.
     */
    protected int[] sortIterationOrder(int[] entryIndexes, CharDoubleComparator comparator) {
    ...
    private final class KeysContainer extends AbstractCharCollection implements CharLookupContainer {
        private final SortedIterationCharDoubleHashMap owner = SortedIterationCharDoubleHashMap.this;
        ...
        public <T extends CharProcedure> T forEach(final T procedure) {
            owner.forEach((CharDoubleProcedure) (k, v) -> procedure.apply(k));
            return procedure;
        }

        @Override
        public <T extends CharPredicate> T forEach(final T predicate) {
            owner.forEach((CharDoublePredicate) (key, value) -> predicate.apply(key));
            return predicate;
        }
    ...
    private final class ValuesContainer extends AbstractDoubleCollection {
        ...
        public <T extends DoubleProcedure> T forEach(T procedure) {
            owner.forEach((CharDoubleProcedure) (k, v) -> procedure.apply(v));
            return procedure;
        }

        @Override
        public <T extends DoublePredicate> T forEach(T predicate) {
            owner.forEach((CharDoublePredicate) (k, v) -> predicate.apply(v));
            return predicate;
        }
