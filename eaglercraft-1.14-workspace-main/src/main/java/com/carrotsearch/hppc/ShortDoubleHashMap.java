...
/**
 * A hash map of <code>short</code> to <code>double</code>, implemented using
...
public class ShortDoubleHashMap implements ShortDoubleMap, Preallocable, Cloneable, Accountable {
    ...
    private final class EntryIterator extends AbstractIterator<ShortDoubleCursor> {
        ...
        protected ShortDoubleCursor fetch() {
            final int mask = ShortDoubleHashMap.this.mask;
            while (index <= mask) {
                short existing;
                index++;
