 ...
 * sorted sequence of <code>float</code>.
 *
 * <p>
...
public class FloatPgmIndex implements Accountable {
     ...
     * an int[].
     */
    public static final int KEY_SIZE = RamUsageEstimator.primitiveSizes.get(float.class) / Integer.BYTES;
    ...
    private FloatPgmIndex(FloatArrayList keys, int size, int epsilon, int epsilonRecursive, int[] levelOffsets,
                          int[] segmentData) {
        assert keys.size() > 0;
    ...
    public static class FloatBuilder implements PlaModel.SegmentConsumer, Accountable {
        ...
        /**
         * Builds the {@link FloatPgmIndex}; or {@link #EMPTY} if there are no keys in
         * the list.
         */
        public FloatPgmIndex build() {
            ...
            return new FloatPgmIndex(keys, size, epsilon, epsilonRecursive, levelOffsetsFinal, segmentDataFinal);
        }
