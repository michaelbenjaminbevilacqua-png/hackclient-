...
/**
 * A hash map of <code>short</code> to <code>Object</code>, implemented using
...
public class ShortObjectHashMap<VType> implements ShortObjectMap<VType>, Preallocable, Cloneable, Accountable {
    /**
    ...
    private final class EntryIterator extends AbstractIterator<ShortObjectCursor<VType>> {
        ...
        protected ShortObjectCursor<VType> fetch() {
            final int mask = ShortObjectHashMap.this.mask;
            while (index <= mask) {
                short existing;
                index++;
