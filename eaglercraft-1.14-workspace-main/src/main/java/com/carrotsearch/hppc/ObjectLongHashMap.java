...
/**
 * A hash map of <code>Object</code> to <code>long</code>, implemented using
...
public class ObjectLongHashMap<KType> implements ObjectLongMap<KType>, Preallocable, Cloneable, Accountable {
    /**
    ...
    private final class EntryIterator extends AbstractIterator<ObjectLongCursor<KType>> {
        ...
        protected ObjectLongCursor<KType> fetch() {
            final int mask = ObjectLongHashMap.this.mask;
            while (index <= mask) {
                KType existing;
                index++;
