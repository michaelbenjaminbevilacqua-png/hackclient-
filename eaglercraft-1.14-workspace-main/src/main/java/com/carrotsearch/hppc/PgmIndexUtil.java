...
final class PgmIndexUtil {
    ...
    /**
     * Adds the intercept of the current segment to the segment data bytes. The
     * intercept is stored as an int for a key size equal to 1, otherwise it is
     * stored as a long.
     *
    ...
    /**
     * Adds the slope of the current segment to the segment data bytes. The
     * intercept is stored as a float for a key size equal to 1, otherwise it is
     * stored as a double.
     *
     * @param keySize The size of the key, measure in {@link Integer#BYTES}.
     */
