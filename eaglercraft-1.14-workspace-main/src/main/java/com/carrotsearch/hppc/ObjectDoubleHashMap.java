...
/**
 * A hash map of <code>Object</code> to <code>double</code>, implemented using
...
public class ObjectDoubleHashMap<KType> implements ObjectDoubleMap<KType>, Preallocable, Cloneable, Accountable {
    /**
    ...
    private final class EntryIterator extends AbstractIterator<ObjectDoubleCursor<KType>> {
        ...
        protected ObjectDoubleCursor<KType> fetch() {
            final int mask = ObjectDoubleHashMap.this.mask;
            while (index <= mask) {
                KType existing;
                index++;
