...
public class CommandDispatcher<S> {
        ...
        if (parse.getReader().canRead()) {
            ...
            for (ArrayList next = null; contexts != null; next = null) {
                ...
                for (int i = 0; i < size; ++i) {
                    CommandContext<S> context = (CommandContext) ((List) contexts).get(i);
    ...
    private void getAllUsage(CommandNode<S> node, S source, ArrayList<String> result, String prefix, boolean restricted) {
        if (!restricted || node.canUse(source)) {
            ...
            if (node.getRedirect() != null) {
                ...
                while (var8.hasNext()) {
                    CommandNode<S> child = (CommandNode) var8.next();
    ...
    public Map<CommandNode<S>, String> getSmartUsage(CommandNode<S> node, S source) {
        ...
        while (var5.hasNext()) {
            CommandNode<S> child = (CommandNode) var5.next();
        ...
        if (!node.canUse(source)) {
            ...
            if (!deep) {
                ...
                if (children.size() == 1) {
                    ...
                    while (var11.hasNext()) {
                        CommandNode<S> child = (CommandNode) var11.next();
                    ...
                    if (childUsage.size() > 1) {
                        ...
                        for (Iterator var20 = children.iterator(); var20.hasNext(); ++count) {
                            CommandNode<S> child = (CommandNode) var20.next();
                            if (count > 0) {
                                builder.append("|");
                            }
    ...
    public CompletableFuture<Suggestions> getCompletionSuggestions(ParseResults<S> parse, int cursor) {
        ...
        final CompletableFuture[] future = new CompletableFuture[1];
        for (Iterator var11 = parent.getChildren().iterator(); var11.hasNext(); futures[i++] = future[0]) {
            CommandNode<S> node = (CommandNode) var11.next();
            future[0] = Suggestions.empty();

            try {
                future[0] = node.listSuggestions(context.build(truncatedInput), new SuggestionsBuilder(truncatedInput, start));
            } catch (CommandSyntaxException var15) {
            }
        }
        ...
        CompletableFuture.allOf(futures).thenRun(() -> {
            ...
            for (CompletableFuture<Suggestions> f : futures) {
                suggestions.add(f.join());
            }
    ...
    public Collection<String> getPath(CommandNode<S> target) {
        ...
        while (var6.hasNext()) {
            CommandNode<S> node = (CommandNode) var6.next();
            if (node != this.root) {
