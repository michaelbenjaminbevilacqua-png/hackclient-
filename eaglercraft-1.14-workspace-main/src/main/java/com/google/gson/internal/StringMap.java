...
public final class StringMap<V> extends AbstractMap<String, V> {
    /**
     * Min capacity (other than zero) for a HashMap. Must be a power of two greater
     * than 1 (and less than 1 << 30).
     */
     ...
     * constructor). It is never written to, and replaced on first put. Its size is
     * set to half the minimum, so that the first resize will create a minimum-sized
     * table.
     */
    ...
    /**
     * A dummy entry in the circular linked list of entries in the map. The first
     * real entry is header.nxt, and the last is header.prv. If the map is empty,
     * header.nxt == header && header.prv == header.
     */
     ...
     * field is generally .75 * capacity, except when the capacity is zero, as
     * described in the EMPTY_TABLE declaration above.
     */
    ...
    private static int hash(String key) {
        ...
        //
        // Finding unpredictable hash functions is a bit of a dark art as we need to
        // balance
        // good unpredictability (to avoid DoS) and good distribution (for performance).
        //
        // We achieve this by using the same algorithm as the Perl version, but this
        // implementation
        ...
        /*
         * Apply Doug Lea's supplemental hash function to avoid collisions for hashes
    ...
    /**
     * Doubles the capacity of the hash table. Existing entries are placed in the
     * correct bucket on the enlarged table. If the current capacity is,
     * MAXIMUM_CAPACITY, this method is a no-op. Returns the table, which will be
     * new unless we were already at MAXIMUM_CAPACITY.
     */
    private LinkedEntry<V>[] doubleCapacity() {
        ...
        for (int j = 0; j < oldCapacity; j++) {
            /*
             * Rehash the bucket using the minimum number of field writes. This is the most
             * subtle and delicate code in the class.
             */
