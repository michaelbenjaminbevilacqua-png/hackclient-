...
public class JsonReader implements Closeable {
        ...
        JsonReaderInternalAccess.INSTANCE = new JsonReaderInternalAccess() {
            ...
            public void promoteNameToValue(JsonReader reader) throws IOException {
                ...
                if (reader.token != JsonToken.NAME) {
                    throw new IllegalStateException("Expected a name but was " + reader.peek() + " " + " at line "
                            + reader.getLineNumber() + " column " + reader.getColumnNumber());
                }
     ...
     * {@link #advance}. If null, peek() will assign a value.
     */
    ...
    /*
     * For the next literal value, we may have the text value, or the position and
    ...
    private void expect(JsonToken expected) throws IOException {
        peek();
        if (token != expected) {
            throw new IllegalStateException("Expected " + expected + " but was " + peek() + " at line "
    ...
    public boolean nextBoolean() throws IOException {
        ...
        if (token != JsonToken.BOOLEAN) {
            throw new IllegalStateException("Expected a boolean but was " + token + " at line " + getLineNumber()
                    + " column " + getColumnNumber());
        }
    ...
    public void nextNull() throws IOException {
        ...
        if (token != JsonToken.NULL) {
            throw new IllegalStateException(
                    "Expected null but was " + token + " at line " + getLineNumber() + " column " + getColumnNumber());
        }
    ...
    public double nextDouble() throws IOException {
        peek();
        if (token != JsonToken.STRING && token != JsonToken.NUMBER) {
            throw new IllegalStateException("Expected a double but was " + token + " at line " + getLineNumber()
                    + " column " + getColumnNumber());
        }
        ...
        if ((result >= 1.0d && value.startsWith("0"))) {
            throw new MalformedJsonException("JSON forbids octal prefixes: " + value + " at line " + getLineNumber()
                    + " column " + getColumnNumber());
        }
        if (!lenient && (Double.isNaN(result) || Double.isInfinite(result))) {
            throw new MalformedJsonException("JSON forbids NaN and infinities: " + value + " at line " + getLineNumber()
                    + " column " + getColumnNumber());
        }
     ...
     * Java {@code long}, this method throws.
     *
     * @throws IllegalStateException if the next token is not a literal value.
     * @throws NumberFormatException if the next literal value cannot be parsed as a
    ...
    public long nextLong() throws IOException {
        peek();
        if (token != JsonToken.STRING && token != JsonToken.NUMBER) {
            throw new IllegalStateException("Expected a long but was " + token + " at line " + getLineNumber()
                    + " column " + getColumnNumber());
        }
        ...
        try {
            ...
            if (result != asDouble) {
                throw new NumberFormatException("Expected a long but was " + value + " at line " + getLineNumber()
                        + " column " + getColumnNumber());
            }
        }
     ...
     * Java {@code int}, this method throws.
     *
     * @throws IllegalStateException if the next token is not a literal value.
     * @throws NumberFormatException if the next literal value cannot be parsed as a
    ...
    public int nextInt() throws IOException {
        peek();
        if (token != JsonToken.STRING && token != JsonToken.NUMBER) {
            throw new IllegalStateException("Expected an int but was " + token + " at line " + getLineNumber()
                    + " column " + getColumnNumber());
        }
        ...
        try {
            ...
            result = (int) asDouble;
            if (result != asDouble) {
                throw new NumberFormatException("Expected an int but was " + value + " at line " + getLineNumber()
                        + " column " + getColumnNumber());
            }
        }
    ...
    private JsonToken nextInObject(boolean firstElement) throws IOException {
        /*
         * Read delimiters. Either a comma/semicolon separating this and the previous
         * name-value pair, or a close brace to denote the end of the object.
         */
        if (firstElement) {
    ...
    private JsonToken objectValue() throws IOException {
        /*
         * Read the name/value separator. Usually a colon ':'. In lenient mode we also
         * accept an equals sign '=', or an arrow "=>".
         */
        switch (nextNonWhitespace(true)) {
            case ':':
                break;
    ...
    /**
     * Reads the value up to but not including any delimiter characters. This does
     * not consume the delimiter character.
     *
     * @param assignOffsetsOnly true for this method to only set the valuePos and
     *                          valueLength fields and return a null result. This
     *                          only works if the literal is short; a string is
     *                          returned otherwise.
     */
    @SuppressWarnings("fallthrough")
    private String nextLiteral(boolean assignOffsetsOnly) throws IOException {
        ...
        while (true) {
            ...
            /*
             * Attempt to load the entire literal into the buffer at once. If we run out of
             * input, add a non-literal character at the end so that decoding doesn't need
             * to do bounds checks.
             */
            if (i < buffer.length) {
                if (fillBuffer(i + 1)) {
            ...
            // use a StringBuilder when the value is too long. It must be an unquoted
            // string.
            if (builder == null) {
                builder = new StringBuilder();
            }
    ...
    /**
     * Determine whether the characters is a JSON number. Numbers are of the form
     * -12.34e+56. Fractional and exponential parts are optional. Leading zeroes are
     * not allowed in the value or exponential part, but are allowed in the
     * fraction.
     */
