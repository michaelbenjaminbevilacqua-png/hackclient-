...
import net.peyton.gson.adapters.ArrayListTypeAdapter;
import net.peyton.gson.adapters.JsonArrayTypeAdapter;
import net.peyton.gson.adapters.JsonObjectTypeAdapter;
import net.peyton.gson.adapters.ResourceLocationAdvancementProgressMapTypeAdapter;
...
public final class Gson {
     ...
     */
    private final ThreadLocal<Map<Class<?>, FutureTypeAdapter<?>>> calls = new ThreadLocal<Map<Class<?>, FutureTypeAdapter<?>>>() {
        @Override
        protected Map<Class<?>, FutureTypeAdapter<?>> initialValue() {
            return new HashMap<Class<?>, FutureTypeAdapter<?>>();
        }
    };

    private final Map<Class<?>, TypeAdapter<?>> typeTokenCache = Collections
            .synchronizedMap(new HashMap<Class<?>, TypeAdapter<?>>());

    private final List<TypeAdapterFactory> factories;
    final JsonDeserializationContext deserializationContext = new JsonDeserializationContext() {
        @SuppressWarnings("unchecked")
        public <T> T deserialize(JsonElement json, Class<T> typeOfT) throws JsonParseException {
     ...
     * <code>&quot;versionNumber@quot;</code> in Json. The same rules are applied
     ...
     */
    public Gson() {
        this(FieldNamingPolicy.IDENTITY, Collections.<Class<?>, InstanceCreator<?>>emptyMap(), false, false,
                DEFAULT_JSON_NON_EXECUTABLE, true, false, false, LongSerializationPolicy.DEFAULT,
                Collections.<TypeAdapterFactory>emptyList());
    }

    Gson(final FieldNamingStrategy fieldNamingPolicy, final Map<Class<?>, InstanceCreator<?>> instanceCreators,
         boolean serializeNulls, boolean complexMapKeySerialization, boolean generateNonExecutableGson,
         boolean htmlSafe, boolean prettyPrinting, boolean serializeSpecialFloatingPointValues,
        ...
        factories.add(JsonObjectTypeAdapter.FACTORY);
        factories.add(JsonArrayTypeAdapter.FACTORY);
        factories.add(ResourceLocationAdvancementProgressMapTypeAdapter.FACTORY);
        factories.add(ArrayListTypeAdapter.FACTORY);
    ...
    public <T> TypeAdapter<T> getAdapter(Class<T> type) {
        ...
        Map<Class<?>, FutureTypeAdapter<?>> threadCalls = calls.get();
     ...
     * }</pre>
     * <p>
     * Note that since you can not override type adapter factories for String and
     * Java primitive types, our stats factory will not count the number of String
     * or primitives that will be read or written.
     *
     ...
     * @since 2.2
     */
    public <T> TypeAdapter<T> getDelegateAdapter(TypeAdapterFactory skipPast, Class<T> type) {
     ...
     */
    @SuppressWarnings("unchecked")
    public void toJson(Object src, JsonWriter writer) throws JsonIOException {
        if (src == null) {
            try {
                writer.nullValue();
            } catch (IOException e) {
        ...
        Class<?> typeOfSrc = src.getClass();
        TypeAdapter<?> adapter = getAdapter(typeOfSrc);
    ...
    public <T> T fromJson(String json, Class<T> classOfT) throws JsonSyntaxException {
        ...
        T target = (T) fromJson(reader, classOfT);
        return target;
    }
     ...
     * type {@code typeOfT}. Since Type is not parameterized by T, this method is
     * type unsafe and should be used carefully
     *
     ...
     */
    @SuppressWarnings("unchecked")
    public <T> T fromJson(JsonReader reader, Class<T> typeOfT) throws JsonIOException, JsonSyntaxException {
        ...
        try {
            reader.peek();
            isEmpty = false;
            TypeAdapter<T> typeAdapter = (TypeAdapter<T>) getAdapter(typeOfT);
     ...
     * @since 1.3
     */
    public <T> T fromJson(JsonElement json, Class<T> typeOfT) throws JsonSyntaxException {
        if (json == null) {
            return null;
        }
    ...
    public String toString() {
        StringBuilder sb = new StringBuilder("{").append("serializeNulls:").append(serializeNulls).append("factories:")
