...
/**
 * An immutable {@code SortedMultiset} that stores its elements in a sorted
 * array. Some instances are ordered by an explicit comparator, while others
 * follow the natural sort ordering of their elements. Either way, null elements
 * are not supported.
 *
 * <p>
 * Unlike {@link Multisets#unmodifiableSortedMultiset}, which is a <i>view</i>
 * of a separate collection that can still change, an instance of
 * {@code ImmutableSortedMultiset} contains its own private data and will
 * <i>never</i> change. This class is convenient for {@code public static
 * final} multisets ("constant multisets") and also lets you easily make a
 * "defensive copy" of a set provided to your class by a caller.
 *
 * <p>
 * The multisets returned by the {@link #headMultiset}, {@link #tailMultiset},
 * and {@link #subMultiset} methods share the same array as the original
 * multiset, preventing that array from being garbage collected. If this is a
 * concern, the data may be copied into a correctly-sized array by calling
 * {@link #copyOfSorted}.
 *
 * <p>
 ...
 * collection. Unlike most collections, an {@code ImmutableSortedMultiset}
 * doesn't use {@link Object#equals} to determine if two elements are
 * equivalent. Instead, with an explicit comparator, the following relation
 * determines whether elements {@code x} and {@code y} are equivalent:
 *
 * <pre>
 *    {@code
 *
 ...
 * </pre>
 *
 * <b>Warning:</b> Like most multisets, an {@code ImmutableSortedMultiset} will
 * not function correctly if an element is modified after being placed in the
 * multiset. For this reason, and to avoid general confusion, it is strongly
 * recommended to place only immutable objects into this collection.
 *
 * <p>
...
public abstract class ImmutableSortedMultiset<E> extends ImmutableSortedMultisetFauxverideShim<E>
    ...
    private static final ImmutableSortedMultiset<Comparable> NATURAL_EMPTY_MULTISET = new EmptyImmutableSortedMultiset<Comparable>(
            NATURAL_ORDER);
    ...
    private static <E> ImmutableSortedMultiset<E> copyOfSortedEntries(Comparator<? super E> comparator,
        ...
        return new RegularImmutableSortedMultiset<E>(
                new RegularImmutableSortedSet<E>(elementsBuilder.build(), comparator), counts, cumulativeCounts, 0,
                entries.size());
    }
     ...
     *
     * <p>
     * Note: the type parameter {@code E} extends {@code Comparable<E>} rather than
     * {@code
