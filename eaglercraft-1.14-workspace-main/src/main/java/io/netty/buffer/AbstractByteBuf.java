...
import static io.netty.buffer.MathUtil.isOutOfBounds;
...
public abstract class AbstractByteBuf extends ByteBuf {
    ...
    public ByteBuf readerIndex(int readerIndex) {
        if (readerIndex < 0 || readerIndex > writerIndex) {
            throw new IndexOutOfBoundsException(HString.format(
    ...
    public ByteBuf writerIndex(int writerIndex) {
        if (writerIndex < readerIndex || writerIndex > capacity()) {
            throw new IndexOutOfBoundsException(
                    HString.format("writerIndex: %d (expected: readerIndex(%d) <= writerIndex <= capacity(%d))",
    ...
    public ByteBuf ensureWritable(int minWritableBytes) {
        if (minWritableBytes < 0) {
            throw new IllegalArgumentException(
                    HString.format("minWritableBytes: %d (expected: >= 0)", minWritableBytes));
        }
    ...
    final void ensureWritable0(int minWritableBytes) {
        ...
        if (minWritableBytes > maxCapacity - writerIndex) {
            throw new IndexOutOfBoundsException(
                    HString.format("writerIndex(%d) + minWritableBytes(%d) exceeds maxCapacity(%d): %s", writerIndex,
    ...
    public String toString() {
        StringBuilder buf = new StringBuilder().append(this.getClass().getSimpleName()).append("(ridx: ")
                .append(readerIndex).append(", widx: ").append(writerIndex).append(", cap: ").append(capacity());
        ...
        /*
         * ByteBuf unwrapped = unwrap(); if (unwrapped != null) {
    ...
    final void checkIndex0(int index, int fieldLength) {
        if (isOutOfBounds(index, fieldLength, capacity())) {
            throw new IndexOutOfBoundsException(
                    HString.format("index: %d, length: %d (expected: range(0, %d))", index, fieldLength, capacity()));
        }
    }
    ...
    protected final void checkSrcIndex(int index, int length, int srcIndex, int srcCapacity) {
        ...
        if (isOutOfBounds(srcIndex, length, srcCapacity)) {
            throw new IndexOutOfBoundsException(
                    HString.format("srcIndex: %d, length: %d (expected: range(0, %d))", srcIndex, length, srcCapacity));
        }
    }
    ...
    protected final void checkDstIndex(int index, int length, int dstIndex, int dstCapacity) {
        ...
        if (isOutOfBounds(dstIndex, length, dstCapacity)) {
            throw new IndexOutOfBoundsException(
                    HString.format("dstIndex: %d, length: %d (expected: range(0, %d))", dstIndex, length, dstCapacity));
        }
    }
    ...
    private void checkReadableBytes0(int minimumReadableBytes) {
        if (readerIndex > writerIndex - minimumReadableBytes) {
            throw new IndexOutOfBoundsException(
                    HString.format("readerIndex(%d) + length(%d) exceeds writerIndex(%d): %s", readerIndex,
