 ...
 *
 * <p>
 * This class can be used as a direct replacement for {@code SimpleDateFormat}
 ...
 * version, nor will it be as Sun has closed the <a href=
...
public class FastDateParser implements DateParser, Serializable {
    ...
    /**
     * Initializes derived fields from defining fields. This is called from
     * constructor and from readObject (de-serialization)
     *
     * @param definingCalendar the {@link Calendar} instance used to
    ...
    /**
     * This implementation updates the ParsePosition if the parse succeeds. However,
     * it sets the error index to the position before the failed field unlike the
     * method {@link java.text.SimpleDateFormat#parse(String, ParsePosition)} which
     * sets the error index to after the failed field.
     * <p>
     * To determine if the parse has succeeded, the caller must check if the current
     * parse position given by {@link ParsePosition#getIndex()} has been updated. If
     * the input buffer has been fully parsed, then the index will point to just
    ...
    private static final Map<Locale, Strategy>[] caches = new Map[Calendar.FIELD_COUNT];
     ...
     */
    private static Map<Locale, Strategy> getCache(final int field) {
        synchronized (caches) {
            if (caches[field] == null) {
                caches[field] = new HashMap<>(3);
            }
            return caches[field];
        }
    }
    ...
    private Strategy getLocaleSpecificStrategy(final int field, final Calendar definingCalendar) {
        final Map<Locale, Strategy> cache = getCache(field);
