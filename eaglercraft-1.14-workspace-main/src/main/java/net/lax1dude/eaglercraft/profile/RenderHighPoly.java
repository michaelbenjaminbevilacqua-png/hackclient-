...
public class RenderHighPoly extends PlayerRenderer {
    ...
    public RenderHighPoly(EntityRendererManager renderManager, EntityModel<?> fallbackModel, float fallbackScale) {
        super(renderManager);
    }

    private static final Matrix4f tmpMatrix = new Matrix4f();

    public void doRender(AbstractClientPlayerEntity abstractclientplayer, double d0, double d1, double d2, float f,
            float f1) {
        if (!abstractclientplayer.isUser() || this.renderManager.info.getRenderViewEntity() == abstractclientplayer) {
            double nameY = d1;
            ...
            try {
                ...
                float f2 = MathHelper.func_219805_h(f1, abstractclientplayer.prevRenderYawOffset, abstractclientplayer.renderYawOffset);
                float f3 = MathHelper.func_219805_h(f1, abstractclientplayer.prevRotationYawHead, abstractclientplayer.rotationYawHead);
                float f4 = f3 - f2;
                if (abstractclientplayer.isPassenger() && abstractclientplayer.getRidingEntity() instanceof LivingEntity) {
                    LivingEntity entitylivingbase1 = (LivingEntity) abstractclientplayer.getRidingEntity();
                    f2 = MathHelper.func_219805_h(f1, entitylivingbase1.prevRenderYawOffset, entitylivingbase1.renderYawOffset);
                    f4 = f3 - f2;
                ...
                if (abstractclientplayer.hurtTime > 0 || abstractclientplayer.deathTime > 0) {
                    GlStateManager.color(1.2f, 0.8F, 0.8F);
                }
                ...
                if (highPolySkin.limbsModel != null && highPolySkin.limbsModel.length > 0) {
                    for (int i = 0; i < highPolySkin.limbsModel.length; ++i) {
                        ...
                        if (i == 0) {
                            ...
                            if (highPolySkin == HighPolySkin.LONG_ARMS) {
                                ...
                                if (stk != null) {
                                    ...
                                    if (itm != null) {
                                        ...
                                        } else if (itm instanceof BlockItem && !Block.getBlockFromItem(itm).getDefaultState().isNormalCube(net.minecraft.world.EmptyBlockReader.INSTANCE, net.minecraft.util.math.BlockPos.ZERO)) {
                                    ...
                                    if (itm != null) {
                                        if (itm == Items.BOW) {
                                            ...
                                            if (!abstractclientplayer.isHandActive()) {
                                                ...
                                                GlStateManager.translate(-0.9f, 0.8f, -0.3f);
                                            } else {
                            ...
                            renderHeldItem(abstractclientplayer, abstractclientplayer.getHeldItemMainhand(), abstractclientplayer.getPrimaryHand());
                            GlStateManager.popMatrix();
                            ...
                            if (highPolySkin == HighPolySkin.LONG_ARMS) {
                                GlStateManager.translate(1.72f, 2.05f, -0.24f);
                                ItemStack stk = abstractclientplayer.getHeldItemOffhand();
                                if (stk != null) {
                                    Item itm = stk.getItem();
                                    if (itm != null) {
                                        if (itm == Items.BOW) {
                            ...
                            renderHeldItem(abstractclientplayer, abstractclientplayer.getHeldItemOffhand(), abstractclientplayer.getPrimaryHand() == HandSide.RIGHT ? HandSide.LEFT : HandSide.RIGHT);
                            GlStateManager.popMatrix();
                        }
                ...
                if (highPolySkin.eyesModel != null) {
                    ...
                    sunCurve = MathHelper.clamp(9.8f - MathHelper.abs(sunCurve * 5.0f + sunCurve * sunCurve * 45.0f - 14.3f) * 0.7f, 0.0f, 1.0f);
    ...
    private void renderHeldItem(AbstractClientPlayerEntity clientPlayer, ItemStack itemstack, HandSide hand) {
        if (itemstack != null) {
            ...
            if (clientPlayer.fishingBobber != null) {
                itemstack = new ItemStack(Items.FISHING_ROD, 0);
            }

            if (item instanceof BlockItem && Block.getBlockFromItem(item).getDefaultState().getRenderType() == net.minecraft.block.BlockRenderType.ENTITYBLOCK_ANIMATED) {
            ...
            mc.getItemRenderer().renderItem(itemstack, clientPlayer, hand == HandSide.RIGHT ? ItemCameraTransforms.TransformType.FIRST_PERSON_RIGHT_HAND : ItemCameraTransforms.TransformType.FIRST_PERSON_LEFT_HAND, hand == HandSide.LEFT);

            GlStateManager.popMatrix();
        }
    }

    public void renderLivingAt(AbstractClientPlayerEntity abstractclientplayer, double d0, double d1, double d2) {
