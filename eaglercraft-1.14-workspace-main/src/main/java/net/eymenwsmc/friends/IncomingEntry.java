...
public class IncomingEntry extends AbstractFriendsEntryContainerWidget {
    ...
    public IncomingEntry(FriendsOverlayScreen screen, String playerName,
        ...
        this.acceptButton = new ImageButton(0, 0, BUTTON_SIZE, BUTTON_SIZE,
                ...
                btn -> {
                    if (!actionInProgress) {
                        ...
                        disable();
                        NetworkHandler.acceptFriend(playerName);
                    }
                }, "") {
            @Override
        ...
        this.rejectButton = new ImageButton(0, 0, BUTTON_SIZE, BUTTON_SIZE,
                ...
                btn -> {
                    if (!actionInProgress) {
                        actionInProgress = true;
                        disable();
                        NetworkHandler.refuseFriend(playerName);
                    }
                }, "") {
            @Override
            public void renderButton(int mx, int my, float pt) {
    ...
    public void render(int mouseX, int mouseY, float partialTicks) {
        ...
        if (acceptButton.isHovered()) {
            screen.setTooltip(I18n.format("gui.friends.accept"), mouseX, mouseY);
            screen.setCursor(EnumCursorType.HAND);
        } else if (rejectButton.isHovered()) {
            screen.setTooltip(I18n.format("gui.friends.reject"), mouseX, mouseY);
            screen.setCursor(EnumCursorType.HAND);
        }
    }
