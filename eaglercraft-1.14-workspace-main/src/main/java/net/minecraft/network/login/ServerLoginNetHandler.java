...
public class ServerLoginNetHandler implements IServerLoginNetHandler {
    ...
    public void processLoginStart(CLoginStartPacket packetIn) {
        ...
        if (this.server.isServerInOnlineMode() && !this.networkManager.isLocalChannel()) {
            ...
            this.networkManager.sendPacket(new SEncryptionRequestPacket("", null, this.verifyToken));
        } else {
            this.currentLoginState = ServerLoginNetHandler.State.READY_TO_ACCEPT;
        }
        ...
        if (!Arrays.equals(this.verifyToken, packetIn.getVerifyToken(privatekey))) {
            ...
            Thread thread = new Thread("User Authenticator #" + AUTHENTICATOR_THREAD_ID.incrementAndGet()) {
                public void run() {
                    ...
                    try {
                        String s = (new BigInteger(CryptManager.getServerIdHash("", null, ServerLoginNetHandler.this.secretKey))).toString(16);
                        ServerLoginNetHandler.this.loginGameProfile = ServerLoginNetHandler.this.server.getMinecraftSessionService().hasJoinedServer(new GameProfile((EaglercraftUUID) null, gameprofile.getName()), s, this.getAddress());
                        if (ServerLoginNetHandler.this.loginGameProfile != null) {
                            ServerLoginNetHandler.LOGGER.info("EaglercraftUUID of player {} is {}", ServerLoginNetHandler.this.loginGameProfile.getName(), ServerLoginNetHandler.this.loginGameProfile.getId());
    ...
    protected GameProfile getOfflineProfile(GameProfile original) {
        EaglercraftUUID uuid = PlayerEntity.getOfflineUUID(original.getName());
