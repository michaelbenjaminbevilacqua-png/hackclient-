...
public class EntityDataManager {
    ...
    private static <T> void writeEntry(PacketBuffer buf, EntityDataManager.DataEntry<T> entry) throws IOException {
        ...
        if (i < 0) {
            throw new IOException("Unknown serializer type " + dataparameter.getSerializer());
        } else {
    ...
    public static List<EntityDataManager.DataEntry<?>> readEntries(PacketBuffer buf) throws IOException {
        ...
        while ((i = buf.readUnsignedByte()) != 255) {
            ...
            if (idataserializer == null) {
                throw new IOException("Unknown serializer type " + j);
            }
    ...
    private <T> void setEntryValue(EntityDataManager.DataEntry<T> target, EntityDataManager.DataEntry<?> source) {
        if (!Objects.equals(source.key.getSerializer(), target.key.getSerializer())) {
            Object tVal = target.value;
            Object sVal = source.value;
            throw new IllegalStateException(String.format("Invalid entity data item type for field %d on entity %s: old=%s(%s), new=%s(%s)", target.key.getId(), this.entity, tVal, tVal != null ? tVal.getClass() : "null", sVal, sVal != null ? sVal.getClass() : "null"));
        } else {
