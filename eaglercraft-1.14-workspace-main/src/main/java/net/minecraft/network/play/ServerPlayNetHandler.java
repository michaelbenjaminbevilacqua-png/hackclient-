...
public class ServerPlayNetHandler implements IServerPlayNetHandler {
    ...
    private final ShortIntMap pendingTransactions = new ShortIntHashMap();
        ...
        if (this.player.openContainer.windowId == packetIn.getWindowId() && this.player.openContainer.getCanCraft(this.player)) {
            if (this.player.isSpectator()) {
                ...
                if (ItemStack.areItemStacksEqual(packetIn.getClickedItem(), itemstack1)) {
                    ...
                    this.pendingTransactions.put((short) this.player.openContainer.windowId, packetIn.getActionNumber());
    ...
    public void processConfirmTransaction(CConfirmTransactionPacket packetIn) {
        ...
        if (i == packetIn.getWindowId() && this.pendingTransactions.getOrDefault((short) i, (short) (packetIn.getUid() + 1)) == packetIn.getUid() && !this.player.openContainer.getCanCraft(this.player) && !this.player.isSpectator()) {
    ...
    public void processCustomPayload(CCustomPayloadPacket packetIn) {
        ...
        try {
            if (eaglerMessageController != null) {
                eaglerMessageController.handlePacket(net.lax1dude.eaglercraft.socket.protocol.GamePluginMessageConstants.fromResourceLocation(channelName), bufferData);
            }
        } catch (java.io.IOException e) {
            LOGGER.error("Couldn't read \"{}\" packet as an eaglercraft plugin message!", channelName);
            LOGGER.error(e);
        }
    }
