...
public class IntArrayNBT extends CollectionNBT<IntNBT> {
    ...
    public void add(int p_add_1_, IntNBT p_add_2_) {
        ...
        int insertIndex = p_add_1_;
        int value = p_add_2_.getInt();
    ...
    public boolean func_218660_b(int p_218660_1_, INBT p_218660_2_) {
        if (p_218660_2_ instanceof NumberNBT) {
            int[] oldArray = this.intArray;
            int insertIndex = p_218660_1_;
            int value = ((NumberNBT) p_218660_2_).getInt();
            int[] newArray = new int[oldArray.length + 1];
            System.arraycopy(oldArray, 0, newArray, 0, insertIndex);
            newArray[insertIndex] = value;
            System.arraycopy(oldArray, insertIndex, newArray, insertIndex + 1, oldArray.length - insertIndex);
    ...
    public IntNBT remove(int p_remove_1_) {
        ...
        int[] oldArray = this.intArray;
        int[] newArray = new int[oldArray.length - 1];
        System.arraycopy(oldArray, 0, newArray, 0, p_remove_1_);
        System.arraycopy(oldArray, p_remove_1_ + 1, newArray, p_remove_1_, oldArray.length - p_remove_1_ - 1);
        this.intArray = newArray;
        return new IntNBT(i);
    }
