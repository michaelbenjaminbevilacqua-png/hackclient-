...
public class ByteArrayNBT extends CollectionNBT<ByteNBT> {
    ...
    public void add(int p_add_1_, ByteNBT p_add_2_) {
        byte[] old = this.data;
        byte[] newArr = new byte[old.length + 1];
        System.arraycopy(old, 0, newArr, 0, p_add_1_);
        newArr[p_add_1_] = p_add_2_.getByte();
        System.arraycopy(old, p_add_1_, newArr, p_add_1_ + 1, old.length - p_add_1_);
        this.data = newArr;
    }
    ...
    public boolean func_218660_b(int p_218660_1_, INBT p_218660_2_) {
        if (p_218660_2_ instanceof NumberNBT) {
            byte[] old = this.data;
            byte[] newArr = new byte[old.length + 1];
            System.arraycopy(old, 0, newArr, 0, p_218660_1_);
            newArr[p_218660_1_] = ((NumberNBT) p_218660_2_).getByte();
            System.arraycopy(old, p_218660_1_, newArr, p_218660_1_ + 1, old.length - p_218660_1_);
            this.data = newArr;
            return true;
        } else {
            return false;
    ...
    public ByteNBT remove(int p_remove_1_) {
        ...
        byte[] old = this.data;
        byte[] newArr = new byte[old.length - 1];
        System.arraycopy(old, 0, newArr, 0, p_remove_1_);
        System.arraycopy(old, p_remove_1_ + 1, newArr, p_remove_1_, old.length - p_remove_1_ - 1);
        this.data = newArr;
        return new ByteNBT(b0);
    }
