package net.minecraft.util;

import com.google.gson.JsonObject;
import com.google.gson.JsonParseException;
import com.mojang.bridge.game.GameVersion;
import net.lax1dude.eaglercraft.EagRuntime;
import net.lax1dude.eaglercraft.EaglercraftUUID;
import org.apache.logging.log4j.LogManager;
import org.apache.logging.log4j.Logger;

import java.io.IOException;
import java.io.InputStream;
import java.io.InputStreamReader;
import java.time.ZonedDateTime;
import java.util.Date;

public class MinecraftVersion implements GameVersion {
    private static final Logger LOGGER = LogManager.getLogger();
    private final String id;
    private final String name;
    private final boolean stable;
    private final int worldVersion;
    private final int protocolVersion;
    private final int packVersion;
    private final Date buildTime;
    private final String releaseTarget;

    public MinecraftVersion() {
        this.id = EaglercraftUUID.randomUUID().toString().replaceAll("-", "");
        this.name = "1.14.4";
        this.stable = true;
        this.worldVersion = 1976;
        this.protocolVersion = 498;
        this.packVersion = 4;
        this.buildTime = new Date();
        this.releaseTarget = "1.14.4";
    }

    protected MinecraftVersion(JsonObject p_i51407_1_) {
        this.id = JSONUtils.getString(p_i51407_1_, "id");
        this.name = JSONUtils.getString(p_i51407_1_, "name");
        this.releaseTarget = JSONUtils.getString(p_i51407_1_, "release_target");
        this.stable = JSONUtils.getBoolean(p_i51407_1_, "stable");
        this.worldVersion = JSONUtils.getInt(p_i51407_1_, "world_version");
        this.protocolVersion = JSONUtils.getInt(p_i51407_1_, "protocol_version");
        this.packVersion = JSONUtils.getInt(p_i51407_1_, "pack_version");
        this.buildTime = new Date(ZonedDateTime.parse(JSONUtils.getString(p_i51407_1_, "build_time")).toInstant().toEpochMilli());
    }

    public static GameVersion load() {
        InputStream inputstream = EagRuntime.getResourceStream("/version.json");
        if (inputstream == null) {
            LOGGER.warn("Missing version information!");
            return new MinecraftVersion();
        }

        MinecraftVersion minecraftversion;
        try {
            minecraftversion = new MinecraftVersion(JSONUtils.fromJson(new InputStreamReader(inputstream)));
        } catch (JsonParseException jsonparseexception) {
            throw new IllegalStateException("Game version information is corrupt", jsonparseexception);
        }

        try {
            inputstream.close();
        } catch (IOException ioexception) {
        }

        return minecraftversion;
    }

    public String getId() {
        return this.id;
    }

    public String getName() {
        return this.name;
    }

    public String getReleaseTarget() {
        return this.releaseTarget;
    }

    public int getWorldVersion() {
        return this.worldVersion;
    }

    public int getProtocolVersion() {
        return this.protocolVersion;
    }

    public int getPackVersion() {
        return this.packVersion;
    }

    public Date getBuildTime() {
        return this.buildTime;
    }

    public boolean isStable() {
        return this.stable;
    }
}
