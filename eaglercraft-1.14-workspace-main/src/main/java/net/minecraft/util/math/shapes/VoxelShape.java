...
public abstract class VoxelShape {
    ...
    protected int getClosestIndex(Direction.Axis axis, double position) {
        int min = 0;
        int max = this.part.getSize(axis) + 1;
        int i = max - min;

        while (i > 0) {
            int j = i / 2;
            int k = min + j;
            if (position < this.getValueUnchecked(axis, k)) {
                i = j;
            } else {
                min = k + 1;
                i -= j + 1;
    ...
    private VoxelShape doProject(Direction side) {
        ...
        if (doublelist.size() == 2 && DoubleMath.fuzzyEquals(doublelist.getDouble(0), 0.0D, 1.0E-5D) && DoubleMath.fuzzyEquals(doublelist.getDouble(1), 1.0D, 1.0E-5D)) {
            return this;
        } else {
            int i = this.getClosestIndex(direction$axis, direction$axisdirection == Direction.AxisDirection.POSITIVE ? 0.9999999D : 1.0E-5D);
    ...
    protected double getAllowedOffset(AxisRotation movementAxis, AxisAlignedBB collisionBox, double desiredOffset) {
        if (this.isEmpty()) {
        ...
        } else if (Math.abs(desiredOffset) < 1.0E-5D) {
            return 0.0D;
        } else {
            ...
            int i = this.getClosestIndex(direction$axis, d1 + 1.0E-5D);
            int j = this.getClosestIndex(direction$axis, d0 - 1.0E-5D);
            int k = Math.max(0, this.getClosestIndex(direction$axis1, collisionBox.getMin(direction$axis1) + 1.0E-5D));
            int l = Math.min(this.part.getSize(direction$axis1), this.getClosestIndex(direction$axis1, collisionBox.getMax(direction$axis1) - 1.0E-5D) + 1);
            int i1 = Math.max(0, this.getClosestIndex(direction$axis2, collisionBox.getMin(direction$axis2) + 1.0E-5D));
            int j1 = Math.min(this.part.getSize(direction$axis2), this.getClosestIndex(direction$axis2, collisionBox.getMax(direction$axis2) - 1.0E-5D) + 1);
