...
public class RecipeItemHelper {
   ...
   public void accountStack(ItemStack stack, int maxCount) {
      if (!stack.isEmpty() && !stack.isDamaged() && !stack.isEnchanted() && !stack.hasDisplayName()) {
         ...
         if (net.minecraft.util.registry.Registry.ITEM.getKey(stack.getItem()) != null && net.minecraft.util.registry.Registry.ITEM.getKey(stack.getItem()).toString().contains("oak_log")) {
             System.out.println("[CRAFT DEBUG] accountStack packed oak_log from grid: " + packed);
         }
         this.itemToCount.put(packed, Math.min(maxCount, stack.getCount()));
      }
   }
   ...
   public boolean canCraft(IRecipe<?> recipe,  IntList packedItemList) {
      return this.canCraft(recipe, packedItemList, 1);
   }

   public boolean canCraft(IRecipe<?> recipe,  IntList packedItemList, int maxAmount) {
      return (new RecipeItemHelper.RecipePicker(recipe)).tryPick(maxAmount, packedItemList);
   }

   public int getBiggestCraftableStack(IRecipe<?> recipe,  IntList packedItemList) {
      return this.getBiggestCraftableStack(recipe, Integer.MAX_VALUE, packedItemList);
   }

   public int getBiggestCraftableStack(IRecipe<?> recipe, int maxAmount,  IntList packedItemList) {
   ...
   class RecipePicker {
      ...
      public boolean tryPick(int maxAmount,  IntList listIn) {
         if (maxAmount <= 0) {
            return true;
         } else {
            int i;
