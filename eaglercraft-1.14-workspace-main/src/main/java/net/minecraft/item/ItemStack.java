...
public final class ItemStack {
    ...
    public boolean attemptDamageItem(int amount, Random rand, ServerPlayerEntity damager) {
    ...
    @OnlyIn(Dist.CLIENT)
    public List<ITextComponent> getTooltip(PlayerEntity playerIn, ITooltipFlag advanced) {
    ...
    private static Collection<ITextComponent> getPlacementTooltip(String stateString) {
        try {
            BlockStateParser blockstateparser = (new BlockStateParser(new com.mojang.brigadier.StringReader(stateString), true)).parse(true);
    ...
    public Multimap<String, AttributeModifier> getAttributeModifiers(EquipmentSlotType equipmentSlot) {
        ...
        if (this.hasTag() && this.tag.contains("AttributeModifiers", 9)) {
            ...
            for (int i = 0; i < listnbt.size(); ++i) {
                ...
                if (attributemodifier != null && (!compoundnbt.contains("Slot", 8) || compoundnbt.getString("Slot").equals(equipmentSlot.getName())) && !attributemodifier.getID().toString().equals("00000000-0000-0000-0000-000000000000")) {
    ...
    public void addAttributeModifier(String attributeName, AttributeModifier modifier, EquipmentSlotType equipmentSlot) {
        this.getOrCreateTag();
    ...
    private static boolean isStateAndTileEntityEqual(CachedBlockInfo p_206846_0_, CachedBlockInfo p_206846_1_) {
        ...
        if (isStateAndTileEntityEqual(p_206848_2_, this.canDestroyCacheBlock)) {
            ...
            if (this.hasTag() && this.tag.contains("CanDestroy", 9)) {
                ...
                for (int i = 0; i < listnbt.size(); ++i) {
                    ...
                    try {
                        Predicate<CachedBlockInfo> predicate = BlockPredicateArgument.blockPredicate().parse(new com.mojang.brigadier.StringReader(s)).create(p_206848_1_);
