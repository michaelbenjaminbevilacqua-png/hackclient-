...
public abstract class PlayerList {
    public static final VFile2 FILE_PLAYERBANS = new VFile2("banned-players.json");
    public static final VFile2 FILE_IPBANS = new VFile2("banned-ips.json");
    public static final VFile2 FILE_OPS = new VFile2("ops.json");
    public static final VFile2 FILE_WHITELIST = new VFile2("whitelist.json");
    private static final Logger LOGGER = LogManager.getLogger();
    private static final SimpleDateFormat DATE_FORMAT = new SimpleDateFormat("yyyy-MM-dd 'at' HH:mm:ss z");
    private final MinecraftServer server;
    private final List<ServerPlayerEntity> players = Lists.newArrayList();
    private final Map<EaglercraftUUID, ServerPlayerEntity> uuidToPlayerMap = Maps.newHashMap();
    ...
    private final Map<EaglercraftUUID, ServerStatisticsManager> playerStatFiles = Maps.newHashMap();
    private final Map<EaglercraftUUID, PlayerAdvancements> advancements = Maps.newHashMap();
    ...
    public void initializeConnectionToPlayer(NetworkManager netManager, ServerPlayerEntity playerIn,
                                             int selectedProtocol, net.lax1dude.eaglercraft.EaglercraftUUID clientBrandUUID) {
        initializeConnectionToPlayer(netManager, playerIn);

    }

    public String allowUserToConnect(GameProfile gameprofile) {
        if (this.getBannedPlayers().isBanned(gameprofile)) {
            return "multiplayer.disconnect.banned.reason";
        } else if (this.whiteListedPlayers.isWhitelisted(gameprofile)) {
            if (!this.whiteListEnforced) {
                return null;
            }
            return "multiplayer.disconnect.whitelisted.reason";
        } else if (this.maxPlayers > 0 && this.players.size() >= this.maxPlayers) {
            return "multiplayer.disconnect.server_full";
        } else {
            return null;
        }
    }
    ...
    public void initializeConnectionToPlayer(NetworkManager netManager, ServerPlayerEntity playerIn) {
        ...
        if (compoundnbt != null && compoundnbt.contains("RootVehicle", 10)) {
            ...
            if (entity1 != null) {
                EaglercraftUUID uuid = compoundnbt1.getUniqueId("Attach");
    ...
    public ServerPlayerEntity createPlayerForUser(GameProfile profile) {
        EaglercraftUUID uuid = PlayerEntity.getUUID(profile);
    ...
    public void sendToAllNearExcept(PlayerEntity except, double x, double y, double z, double radius, DimensionType dimension, IPacket<?> packetIn) {
        for (int i = 0; i < this.players.size(); ++i) {
            ServerPlayerEntity serverplayerentity = this.players.get(i);
    ...
    public ServerStatisticsManager getPlayerStats(PlayerEntity playerIn) {
        ...
        if (serverstatisticsmanager == null) {
            VFile2 file1 = new VFile2(this.server.getWorld(DimensionType.OVERWORLD).getSaveHandler().getWorldDirectory(), "stats");
            VFile2 file2 = new VFile2(file1, uuid + ".json");
            if (!file2.exists()) {
                VFile2 file3 = new VFile2(file1, playerIn.getName().getString() + ".json");
    ...
    public PlayerAdvancements getPlayerAdvancements(ServerPlayerEntity p_192054_1_) {
        ...
        if (playeradvancements == null) {
            VFile2 file1 = new VFile2(this.server.getWorld(DimensionType.OVERWORLD).getSaveHandler().getWorldDirectory(), "advancements");
            VFile2 file2 = new VFile2(file1, uuid + ".json");
