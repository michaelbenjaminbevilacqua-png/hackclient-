...
public class PreYggdrasilConverter {
    ...
    public static final VFile2 OLD_IPBAN_FILE = new VFile2("banned-ips.txt");
    public static final VFile2 OLD_PLAYERBAN_FILE = new VFile2("banned-players.txt");
    public static final VFile2 OLD_OPS_FILE = new VFile2("ops.txt");
    public static final VFile2 OLD_WHITELIST_FILE = new VFile2("white-list.txt");

    static List<String> readFile(VFile2 inFile, Map<String, String[]> read) throws IOException {
    ...
    private static void lookupNames(MinecraftServer server, Collection<String> names, ProfileLookupCallback callback) {
        ...
        if (server.isServerInOnlineMode()) {
            ...
            for (String s : astring) {
                EaglercraftUUID uuid = PlayerEntity.getUUID(new GameProfile((EaglercraftUUID) null, s));
    ...
    public static boolean convertUserBanlist(final MinecraftServer server) {
        ...
        if (OLD_PLAYERBAN_FILE.exists() && OLD_PLAYERBAN_FILE.exists()) {
    ...
    public static boolean convertIpBanlist(MinecraftServer server) {
        ...
        if (OLD_IPBAN_FILE.exists() && OLD_IPBAN_FILE.exists()) {
    ...
    public static boolean convertOplist(final MinecraftServer server) {
        ...
        if (OLD_OPS_FILE.exists() && OLD_OPS_FILE.exists()) {
    ...
    public static boolean convertWhitelist(final MinecraftServer server) {
        ...
        if (OLD_WHITELIST_FILE.exists() && OLD_WHITELIST_FILE.exists()) {
    ...
    public static String convertMobOwnerIfNeeded(final MinecraftServer server, String username) {
        if (!StringUtils.isNullOrEmpty(username) && username.length() <= 16) {
            ...
            if (gameprofile != null && gameprofile.getId() != null) {
                ...
                return PlayerEntity.getUUID(new GameProfile((EaglercraftUUID) null, username)).toString();
            }
        } else {
            return username;
        }
    ...
    public static boolean convertSaveFiles(final DedicatedServer server) {
        final VFile2 file1 = func_219585_g(server);
        final VFile2 file2 = new VFile2((file1.getParent() != null ? new net.lax1dude.eaglercraft.internal.vfs2.VFile2(file1.getParent()) : null), "playerdata");
        final VFile2 file3 = new VFile2((file1.getParent() != null ? new net.lax1dude.eaglercraft.internal.vfs2.VFile2(file1.getParent()) : null), "unknownplayers");
        if (file1.exists() && file1.dirExists()) {
            net.lax1dude.eaglercraft.internal.vfs2.VFile2[] afile = file1.listFiles(false).toArray(new net.lax1dude.eaglercraft.internal.vfs2.VFile2[0]);
            List<String> list = Lists.newArrayList();

            for (VFile2 file4 : afile) {
            ...
            try {
                ...
                ProfileLookupCallback profilelookupcallback = new ProfileLookupCallback() {
                    public void onProfileLookupSucceeded(GameProfile p_onProfileLookupSucceeded_1_) {
                        server.getPlayerProfileCache().addEntry(p_onProfileLookupSucceeded_1_);
                        EaglercraftUUID uuid = p_onProfileLookupSucceeded_1_.getId();
                        if (uuid == null) {
                            throw new PreYggdrasilConverter.ConversionError("Missing EaglercraftUUID for user profile " + p_onProfileLookupSucceeded_1_.getName());
                        } else {
                    ...
                    private void renamePlayerFile(VFile2 p_152743_1_, String p_152743_2_, String p_152743_3_) {
                        VFile2 file5 = new VFile2(file1, p_152743_2_ + ".dat");
                        VFile2 file6 = new VFile2(p_152743_1_, p_152743_3_ + ".dat");
    ...
    private static void mkdir(VFile2 dir) {
        if (dir.exists()) {
            if (!dir.dirExists()) {
    ...
    private static boolean func_219589_f(MinecraftServer p_219589_0_) {
        VFile2 file1 = func_219585_g(p_219589_0_);
        if (!file1.exists() || !file1.dirExists() || file1.listFiles(false).size() <= 0 && file1.delete()) {
            return true;
        } else {
    ...
    private static VFile2 func_219585_g(MinecraftServer p_219585_0_) {
        String s = p_219585_0_.getFolderName();
        VFile2 file1 = new VFile2(s);
        return new VFile2(file1, "players");
    }

    private static void backupConverted(VFile2 convertedFile) {
        VFile2 file1 = new VFile2(convertedFile.getName() + ".converted");
