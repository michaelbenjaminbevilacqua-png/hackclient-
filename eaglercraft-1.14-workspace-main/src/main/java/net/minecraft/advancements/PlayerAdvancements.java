...
public class PlayerAdvancements {
   ...
   private final net.lax1dude.eaglercraft.internal.vfs2.VFile2 progressFile;
   ...
   public PlayerAdvancements(MinecraftServer server, net.lax1dude.eaglercraft.internal.vfs2.VFile2 p_i47422_2_, ServerPlayerEntity player) {
      this.server = server;
   ...
   private void ensureAllVisible() {
      ...
      for(Advancement advancement : this.server.getAdvancementManager().getAllAdvancements()) {
         if (advancement.getParent() == null && advancement.getDisplay() != null && !advancement.getDisplay().isHidden()) {
   ...
   private void load() {
      if (this.progressFile.exists()) {
         ...
         try {
            ...
            for (Map.Entry<String, JsonElement> entry : jsonobject.entrySet()) {
               ...
               AdvancementProgress advancementprogress = GSON.fromJson(entry.getValue(), AdvancementProgress.class);
      ...
      Advancement storyRoot = this.server.getAdvancementManager().getAdvancement(new ResourceLocation("minecraft:story/root"));
      if(storyRoot != null) {
         for(String s : storyRoot.getCriteria().keySet()) {
            this.grantCriterion(storyRoot, s);
         }
      }
   ...
   public void save() {
      ...
      try (
         OutputStream outputstream = this.progressFile.getOutputStream();
