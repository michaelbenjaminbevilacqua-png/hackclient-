...
public class CrashReport {
    ...
    private VFile2 crashReportFile;
    private boolean firstCategoryInCrashReport = true;
    private StackTraceElement[] stacktrace = new StackTraceElement[0];

    public CrashReport(String descriptionIn, Throwable causeThrowable) {
        this.description = descriptionIn;
        this.cause = causeThrowable == null ? new Throwable("Unknown/null exception") : causeThrowable;
    ...
    private void populateEnvironment() {
        ...
        this.systemDetailsCategory.addDetail("Memory", () -> {
            return "TeaVM (Browser) Memory";
        });
        this.systemDetailsCategory.addDetail("CPUs", 1);
    ...
    public void getSectionsInStringBuilder(StringBuilder builder) {
        if ((this.stacktrace == null || this.stacktrace.length <= 0) && !this.crashReportSections.isEmpty()) {
            StackTraceElement[] fullStack = this.crashReportSections.get(0).getStackTrace();
            this.stacktrace = new StackTraceElement[Math.min(1, fullStack.length)];
            System.arraycopy(fullStack, 0, this.stacktrace, 0, this.stacktrace.length);
        }
    ...
    @OnlyIn(Dist.CLIENT)
    public VFile2 getFile() {
        return this.crashReportFile;
    }

    public boolean saveToFile(VFile2 toFile) {
        if (this.crashReportFile != null) {
            ...
            boolean flag1;
            try {
                writer = new OutputStreamWriter(toFile.getOutputStream(), StandardCharsets.UTF_8);
    ...
    public static CrashReport makeCrashReport(Throwable causeIn, String descriptionIn) {
        if (causeIn == null) {
            causeIn = new Throwable("Unknown/null exception");
        }

        while (causeIn.getCause() != null) {
