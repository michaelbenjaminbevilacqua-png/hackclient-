    ...
    public static BlockPattern getOrCreatePortalShape() {
        if (portalShape == null) {
            portalShape = BlockPatternBuilder.start()
                    ...
                    .where('^', CachedBlockInfo.hasState(BlockStateMatcher.forBlock(Blocks.END_PORTAL_FRAME)
                            .where(EYE, eye -> Boolean.TRUE.equals(eye))
                            .where(FACING, facing -> facing == Direction.SOUTH)))
                    .where('>', CachedBlockInfo.hasState(BlockStateMatcher.forBlock(Blocks.END_PORTAL_FRAME)
                            .where(EYE, eye -> Boolean.TRUE.equals(eye))
                            .where(FACING, facing -> facing == Direction.WEST)))
                    .where('v', CachedBlockInfo.hasState(BlockStateMatcher.forBlock(Blocks.END_PORTAL_FRAME)
                            .where(EYE, eye -> Boolean.TRUE.equals(eye))
                            .where(FACING, facing -> facing == Direction.NORTH)))
                    .where('<', CachedBlockInfo.hasState(BlockStateMatcher.forBlock(Blocks.END_PORTAL_FRAME)
                            .where(EYE, eye -> Boolean.TRUE.equals(eye))
                            .where(FACING, facing -> facing == Direction.EAST)))
                    .build();
        }
        return portalShape;
    }
