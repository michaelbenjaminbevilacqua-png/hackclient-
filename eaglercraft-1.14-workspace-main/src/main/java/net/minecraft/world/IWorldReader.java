...
public interface IWorldReader extends IEnviromentBlockReader {
    ...
    boolean checkNoEntityCollision(Entity entityIn, VoxelShape shape);
    ...
    default boolean isCollisionBoxesEmpty(Entity entityIn, AxisAlignedBB aabb, Set<Entity> entitiesToIgnore) {
        return this.getCollisionShapes(entityIn, aabb, entitiesToIgnore).allMatch(VoxelShape::isEmpty);
    }

    default Stream<VoxelShape> getEmptyCollisionShapes(Entity entityIn, AxisAlignedBB aabb, Set<Entity> entitiesToIgnore) {
        return Stream.empty();
    }

    default Stream<VoxelShape> getCollisionShapes(Entity entityIn, AxisAlignedBB aabb, Set<Entity> entitiesToIgnore) {
        return Streams.concat(this.getCollisionShapes(entityIn, aabb), this.getEmptyCollisionShapes(entityIn, aabb, entitiesToIgnore));
    }

    default Stream<VoxelShape> getCollisionShapes(final Entity entityIn, AxisAlignedBB aabb) {
        int i = MathHelper.floor(aabb.minX - 1.0E-5D) - 1;
        int j = MathHelper.floor(aabb.maxX + 1.0E-5D) + 1;
        int k = MathHelper.floor(aabb.minY - 1.0E-5D) - 1;
        int l = MathHelper.floor(aabb.maxY + 1.0E-5D) + 1;
        int i1 = MathHelper.floor(aabb.minZ - 1.0E-5D) - 1;
        int j1 = MathHelper.floor(aabb.maxZ + 1.0E-5D) + 1;
        ...
        return StreamSupport.stream(java.util.Spliterators.spliteratorUnknownSize(new java.util.Iterator<VoxelShape>() {
            boolean isEntityNull = entityIn == null;
            VoxelShape nextShape = null;

            public boolean hasNext() {
                if (nextShape != null) return true;

                if (!this.isEntityNull) {
                    this.isEntityNull = true;
                    VoxelShape voxelshape1 = IWorldReader.this.getWorldBorder().getShape();
                    boolean flag = VoxelShapes.compare(voxelshape1, VoxelShapes.create(entityIn.getBoundingBox().shrink(1.0E-5D)), IBooleanFunction.AND);
                    boolean flag1 = VoxelShapes.compare(voxelshape1, VoxelShapes.create(entityIn.getBoundingBox().grow(1.0E-5D)), IBooleanFunction.AND);
                    if (!flag && flag1) {
                        nextShape = voxelshape1;
                        return true;
                    }
                }
                    ...
                    if (k1 != 3) {
                        ...
                        if (ichunk != null) {
                            ...
                            if (k1 < 3) {
                                ...
                                VoxelShape voxelshape3 = voxelshape2.withOffset((double) j2, (double) k2, (double) l2);
                                if (!voxelshape3.isEmpty() && aabb.intersects(voxelshape3.getBoundingBox())) {
                                    nextShape = voxelshape3;
                                    return true;
                                }
                            }
                        }
