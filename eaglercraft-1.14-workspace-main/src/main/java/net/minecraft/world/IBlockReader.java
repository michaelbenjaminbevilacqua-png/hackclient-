...
public interface IBlockReader {
    ...
    static <T> T func_217300_a(RayTraceContext p_217300_0_, BiFunction<RayTraceContext, BlockPos, T> p_217300_1_, Function<RayTraceContext, T> p_217300_2_) {
        ...
        if (vec3d.equals(vec3d1)) {
            ...
            double d0 = MathHelper.lerp(-1.0E-5D, vec3d1.x, vec3d.x);
            double d1 = MathHelper.lerp(-1.0E-5D, vec3d1.y, vec3d.y);
            double d2 = MathHelper.lerp(-1.0E-5D, vec3d1.z, vec3d.z);
            double d3 = MathHelper.lerp(-1.0E-5D, vec3d.x, vec3d1.x);
            double d4 = MathHelper.lerp(-1.0E-5D, vec3d.y, vec3d1.y);
            double d5 = MathHelper.lerp(-1.0E-5D, vec3d.z, vec3d1.z);
