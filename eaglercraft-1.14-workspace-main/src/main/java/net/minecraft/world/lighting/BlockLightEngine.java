    ...
    protected int getEdgeLevel(long startPos, long endPos, int startLevel) {
        if (endPos == Long.MAX_VALUE) {
            ...
            if (direction == null) {
                ...
                BlockState blockstate = this.getBlockAndOpacity(endPos);
                int opacity = this.pendingOpacity;
                if (opacity >= 15) {
                    return 15;
                } else {
                    ...
                    return VoxelShapes.func_223416_b(voxelshape, voxelshape1) ? 15 : startLevel + Math.max(1, opacity);
                }
            }
        }
    }
