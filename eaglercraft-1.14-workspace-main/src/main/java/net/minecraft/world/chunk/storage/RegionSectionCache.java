...
public class RegionSectionCache<R extends IDynamicSerializable> extends RegionFileCache {
    ...
    private <T> void func_219119_a(ChunkPos p_219119_1_, DynamicOps<T> p_219119_2_, T p_219119_3_) {
        if (p_219119_3_ == null) {
            for (int i = 0; i < 16; ++i) {
                this.data.put(SectionPos.from(p_219119_1_, i).asLong(), Optional.empty());
            }
        } else {
            Dynamic<T> dynamic = new Dynamic<>(p_219119_2_, p_219119_3_);
            int j = func_219103_a(dynamic);
            ...
            for (int l = 0; l < 16; ++l) {
                ...
                Optional<R> optional = optionaldynamic.get(Integer.toString(l)).get().map((p_219105_3_) -> {
                    return this.field_219123_e.apply(() -> {
                        this.markDirty(i1);
                    }, p_219105_3_);
                });
    ...
    private <T> Dynamic<T> serialize(ChunkPos p_219108_1_, DynamicOps<T> p_219108_2_) {
        ...
        for (int i = 0; i < 16; ++i) {
            ...
            if (optional != null && optional.isPresent()) {
                map.put(p_219108_2_.createString(Integer.toString(i)), optional.get().serialize(p_219108_2_));
            }
        }
    ...
    private static int func_219103_a(Dynamic<?> p_219103_0_) {
        return p_219103_0_.get("DataVersion").asInt(1945);
    }
