...
public class RegionFile implements AutoCloseable {
    ...
    private boolean convertChunks = false;

    public RegionFile(VFile2 fileNameIn) {
        this.convertChunks = true;
        fileName = fileNameIn.getPath();
        byte[] data = fileNameIn.getAllBytes();
        if (data != null) {
            this.dataFile = new RandomAccessMemoryFile(data, data.length);
        } else {
            this.dataFile = new RandomAccessMemoryFile(new byte[0], 0);
        }
        setupRegionFile();
    }

    public RegionFile(RandomAccessMemoryFile fileNameIn) {
        this.dataFile = fileNameIn;
        setupRegionFile();
    }
    ...
    private void setupRegionFile() {
        try {
            ...
            if ((this.dataFile.getLength() & 4095L) != 0L) {
                for (int i = 0; (long) i < (this.dataFile.getLength() & 4095L); ++i) {
                    this.dataFile.write(0);
                }
            }

            int i1 = (int) this.dataFile.getLength() / 4096;
                        ...
                        if (l > 4096 * k) {
                            ...
                            if (b0 == 1) {
                                ...
                                return new DataInputStream(
                                        new BufferedInputStream(EaglerZLIB.newGZIPInputStream(new ByteArrayInputStream(abyte1))));
                            } else if (b0 == 2) {
    ...
    public DataOutputStream getChunkDataOutputStream(int x, int z) throws IOException {
        return this.outOfBounds(x, z) ? null
                ...
                new BufferedOutputStream(EaglerZLIB.newDeflaterOutputStream(new RegionFile.ChunkBuffer(x, z))));
    }
    ...
    public DataInputStream func_222666_a(net.minecraft.util.math.ChunkPos pos) throws IOException {
        return getChunkDataInputStream(pos.getRegionPositionX(), pos.getRegionPositionZ());
    }

    public DataOutputStream func_222661_c(net.minecraft.util.math.ChunkPos pos) throws IOException {
        return getChunkDataOutputStream(pos.getRegionPositionX(), pos.getRegionPositionZ());
    }

    public boolean func_222662_b(net.minecraft.util.math.ChunkPos pos) {
        return isChunkSaved(pos.getRegionPositionX(), pos.getRegionPositionZ());
    }
    ...
    public void close() throws IOException {
        if (fileName != null && dataFile != null) {
            new VFile2(fileName).setAllBytes(dataFile.getByteArray());
        }
    }
