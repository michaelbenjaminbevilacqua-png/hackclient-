...
public class DimensionSavedDataManager {
    ...
    public DimensionSavedDataManager(VFile2 dataFolder, DataFixer dataFixerIn) {
        this.dataFixer = dataFixerIn;
        this.folder = dataFolder;
    }

    private VFile2 getDataFile(String name) {
        return new VFile2(this.folder, name + ".dat");
    }
    ...
    private <T extends WorldSavedData> T func_223409_c(Supplier<T> p_223409_1_, String p_223409_2_) {
        try {
            VFile2 file1 = this.getDataFile(p_223409_2_);
            if (file1.exists()) {
                T t = p_223409_1_.get();
    ...
    public CompoundNBT load(String name, int worldVersion) throws IOException {
        VFile2 file1 = this.getDataFile(name);

        CompoundNBT compoundnbt1;
        try (PushbackInputStream pushbackinputstream = new PushbackInputStream(file1.getInputStream(), 2)) {
