...
public class SetLore extends LootFunction {
    ...
    public SetLore(ILootCondition[] p_i51220_1_, boolean replace, List<ITextComponent> lore, LootContext.EntityTarget p_i51220_4_) {
        super(p_i51220_1_);
    ...
    public static class Serializer extends LootFunction.Serializer<SetLore> {
        ...
        public SetLore deserialize(JsonObject object, JsonDeserializationContext deserializationContext, ILootCondition[] conditionsIn) {
            ...
            List<ITextComponent> list = Streams.stream(JSONUtils.getJsonArray(object, "lore")).map(ITextComponent.Serializer::fromJson).collect(Collectors.toList());
