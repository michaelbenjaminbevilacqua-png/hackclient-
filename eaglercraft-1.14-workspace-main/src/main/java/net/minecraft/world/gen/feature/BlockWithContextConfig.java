...
public class BlockWithContextConfig implements IFeatureConfig {
    ...
    public static <T> BlockWithContextConfig deserialize(Dynamic<T> p_214663_0_) {
        BlockState blockstate = BlockState.deserialize(p_214663_0_.get("to_place").orElseEmptyMap());
        List<BlockState> list = p_214663_0_.get("place_on").asList(dyn -> BlockState.deserialize(dyn));
        List<BlockState> list1 = p_214663_0_.get("place_in").asList(dyn -> BlockState.deserialize(dyn));
        List<BlockState> list2 = p_214663_0_.get("place_under").asList(dyn -> BlockState.deserialize(dyn));
