...
public class KeyboardListener {
    ...
    private void setBlockClipboardString(BlockState state, BlockPos pos, CompoundNBT compound) {
        if (compound != null) {
            compound.remove("x");
            compound.remove("y");
            compound.remove("z");
    ...
    private void setEntityClipboardString(ResourceLocation entityIdIn, Vec3d pos, CompoundNBT compound) {
        String s;
        if (compound != null) {
        ...
        if (this.mc.currentScreen == null || this.mc.currentScreen.passEvents) {
            ...
            if (action == 0) {
                ...
                if (this.mc.currentScreen == null) {
                    ...
                    flag1 = InputMappings.isKeyDown(292) && this.processKeyF3(key);
    ...
    public void setClipboardString(String string) {
        EagRuntime.setClipboard(string);
    }
    ...
    public void tick() {
        while (Keyboard.next()) {
            ...
            int glfwKey = net.lax1dude.eaglercraft.KeyboardConstants.getGLFWKeyFromEagler(eaglerKey);
            int action = pressed ? 1 : 0;
            this.onKeyEvent(0L, glfwKey, 0, action, 0);
            if (eventChar != 0 && pressed) {
                this.onCharEvent(0L, eventChar, 0);
            }
        }
