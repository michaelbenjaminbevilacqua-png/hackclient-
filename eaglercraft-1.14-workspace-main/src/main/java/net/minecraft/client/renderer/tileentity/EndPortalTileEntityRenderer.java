...
public class EndPortalTileEntityRenderer extends TileEntityRenderer<EndPortalTileEntity> {
    ...
    private static final float[] MODELVIEW_ARRAY = new float[16];
    ...
    public void render(EndPortalTileEntity tileEntityIn, double x, double y, double z, float partialTicks, int destroyStage) {
        ...
        for (int j = 0; j < i; ++j) {
            ...
            PROJECTION.rewind();
            PROJECTION.get(PROJECTION_ARRAY);
            MODELVIEW.rewind();
            MODELVIEW.get(MODELVIEW_ARRAY);
            GlStateManager.multMatrix(PROJECTION_ARRAY);
            GlStateManager.multMatrix(MODELVIEW_ARRAY);
