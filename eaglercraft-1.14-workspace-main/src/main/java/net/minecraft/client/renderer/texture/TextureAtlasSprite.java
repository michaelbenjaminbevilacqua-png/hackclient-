...
public class TextureAtlasSprite {
    ...
    protected TextureAtlasSprite(ResourceLocation locationIn, PngSizeInfo sizeIn, AnimationMetadataSection animationMetadataIn) {
        this.iconName = locationIn;
    ...
    public void loadSpriteFrames(IResource resource, int count) throws IOException {
        ...
        if (this.interpolatedFrameData != null) {
            for (NativeImage oldFrame : this.interpolatedFrameData) {
                if (oldFrame != null) {
                    oldFrame.close();
                }
            }
        ...
        if (nativeimage.getWidth() < this.width || nativeimage.getHeight() < this.height) {
            NativeImage scaled = new NativeImage(this.width, this.height, true);
            nativeimage.resizeSubRectTo(0, 0, nativeimage.getWidth(), nativeimage.getHeight(), scaled);
            nativeimage.close();
            nativeimage = scaled;
        }
