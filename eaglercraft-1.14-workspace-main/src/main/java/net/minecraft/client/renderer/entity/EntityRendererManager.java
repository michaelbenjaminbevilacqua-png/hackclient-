...
public class EntityRendererManager {
   ...
   public EntityRendererManager(TextureManager p_i50971_1_, net.minecraft.client.renderer.ItemRenderer p_i50971_2_, IReloadableResourceManager p_i50971_3_) {
      ...
      if (EagRuntime.getConfiguration().isAllowFNAWSkins()) {
         this.eaglerRenderer = new RenderHighPoly(this, this.playerRenderer.getEntityModel(), this.playerRenderer.shadowSize);
      } else {
         this.eaglerRenderer = this.playerRenderer;
      }
      this.skinMap.put("eagler", Minecraft.getInstance().gameSettings.enableFNAWSkins ? this.eaglerRenderer : this.playerRenderer);
   }
   ...
   public void setEnableFNAWSkins(boolean enable) {
      this.skinMap.put("eagler", enable ? this.eaglerRenderer : this.playerRenderer);
   }
}
