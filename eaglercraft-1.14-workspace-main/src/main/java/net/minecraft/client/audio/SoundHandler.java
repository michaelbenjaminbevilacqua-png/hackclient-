...
public class SoundHandler extends ReloadListener<SoundHandler.Loader> {
   ...
   protected static Map<String, SoundList> getSoundMap(InputStream p_175085_0_) {
      ...
      try {
         com.google.gson.JsonObject jsonObject = GSON.fromJson(new InputStreamReader(p_175085_0_, StandardCharsets.UTF_8), com.google.gson.JsonObject.class);
         ...
         if (jsonObject != null) {
            for (java.util.Map.Entry<String, com.google.gson.JsonElement> entry : jsonObject.entrySet()) {
               ...
               SoundList soundList = GSON.fromJson(element, SoundList.class);
               if (soundList != null) {
                  finalMap.put(key, soundList);
               }
            }
         }
      } catch (Throwable t) {
         LOGGER.error("TeaVM Friendly GSON parsing failed for sounds.json", t);
      } finally {
   ...
   private static boolean func_215292_b(Sound p_215292_0_, ResourceLocation p_215292_1_, IResourceManager p_215292_2_) {
      ...
      if (!p_215292_2_.hasResource(resourcelocation)) {
         LOGGER.warn("VFile2 {} does not exist, cannot add it to event {}", resourcelocation, p_215292_1_);
         return false;
      } else {
         return true;
      }
   ...
   public void stop( ResourceLocation id,  SoundCategory category) {
   ...
   public static class SoundMapWrapper {
      public Map<String, SoundList> map = com.google.common.collect.Maps.newHashMap();
   }
}
