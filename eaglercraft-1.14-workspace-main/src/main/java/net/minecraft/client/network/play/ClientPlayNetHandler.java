...
public class ClientPlayNetHandler implements IClientPlayNetHandler {
    ...
    private final EaglercraftUUID field_217289_q = EaglercraftUUID.randomUUID();

    public boolean currentFNAWSkinAllowedState = true;
    public boolean currentFNAWSkinForcedState = true;
    private net.lax1dude.eaglercraft.profile.ServerSkinCache skinCache;
    private net.lax1dude.eaglercraft.profile.ServerCapeCache capeCache;
    private net.lax1dude.eaglercraft.notifications.ServerNotificationManager notifManager;
    ...
    public ClientPlayNetHandler(Minecraft mcIn, Screen p_i46300_2_, NetworkManager networkManagerIn, GameProfile profileIn) {
        ...
        this.skinCache = new net.lax1dude.eaglercraft.profile.ServerSkinCache(this, mcIn.getTextureManager());
        this.capeCache = new net.lax1dude.eaglercraft.profile.ServerCapeCache(this, mcIn.getTextureManager());
        this.notifManager = new net.lax1dude.eaglercraft.notifications.ServerNotificationManager();
    }

    public net.lax1dude.eaglercraft.profile.ServerSkinCache getSkinCache() {
        return this.skinCache;
    }

    public net.lax1dude.eaglercraft.profile.ServerCapeCache getCapeCache() {
        return this.capeCache;
    }

    public net.lax1dude.eaglercraft.notifications.ServerNotificationManager getNotifManager() {
        return this.notifManager;
    }

    private net.lax1dude.eaglercraft.socket.protocol.client.GameProtocolMessageController eaglerMessageController = null;

    public net.lax1dude.eaglercraft.socket.protocol.client.GameProtocolMessageController getEaglerMessageController() {
        return eaglerMessageController;
    }

    public void setEaglerMessageController(net.lax1dude.eaglercraft.socket.protocol.client.GameProtocolMessageController controller) {
        this.eaglerMessageController = controller;
    }

    public net.lax1dude.eaglercraft.socket.protocol.GamePluginMessageProtocol getEaglerMessageProtocol() {
    ...
    public boolean isClientInEaglerSingleplayer() {
        return this.netManager instanceof net.minecraft.network.SingleplayerNetworkManager;
    }
    ...
    public void handleUpdateRecipes(SUpdateRecipesPacket packetIn) {
        PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.client);
        Collection<IRecipe<?>> recipes = packetIn.getRecipes();
        if (recipes.isEmpty()) {
            java.util.function.Supplier<java.util.Collection<IRecipe<?>>> fallback = RecipeManager.getRecipeFallback();
            if (fallback != null) {
                recipes = fallback.get();
                if (recipes.isEmpty()) {
                    recipes = packetIn.getRecipes();
                }
            }
        }
        this.recipeManager.func_223389_a(recipes);
    ...
    public void handleRecipeBook(SRecipeBookPacket packetIn) {
        ...
        switch (srecipebookpacket$state) {
            case REMOVE:
                ...
                break;
            case INIT:
                java.util.function.Supplier<java.util.Collection<IRecipe<?>>> fb;
                if (this.recipeManager.getRecipes().isEmpty()
                        && (fb = RecipeManager.getRecipeFallback()) != null) {
                    java.util.Collection<IRecipe<?>> fallbackRecipes = fb.get();
                    if (!fallbackRecipes.isEmpty()) {
                        this.recipeManager.func_223389_a(fallbackRecipes);
                        IMutableSearchTree<RecipeList> st = this.client.func_213253_a(SearchTreeManager.RECIPES);
                        st.func_217871_a();
                        clientrecipebook.rebuildTable();
                        clientrecipebook.getRecipes().forEach(st::func_217872_a);
                        st.recalculate();
                    }
                }
        ...
        if (this.validateResourcePackUrl(s)) {
            if (s.startsWith("level://")) {
                try {
                    ...
                    VFile2 file1 = new VFile2(this.client.gameDir, "saves");
    ...
    public void handleCustomPayload(SCustomPayloadPlayPacket packetIn) {
        ...
        try {
            ...
            if (SCustomPayloadPlayPacket.BRAND.equals(resourcelocation)) {
                ...
                EaglercraftUUID uuid = packetbuffer.readUniqueId();
            ...
            if (eaglerMessageController != null) {
                eaglerMessageController.handlePacket(net.lax1dude.eaglercraft.socket.protocol.GamePluginMessageConstants.fromResourceLocation(resourcelocation.toString()), packetbuffer);
            }
        } catch (java.io.IOException e) {
            LOGGER.error("Couldn't read \"{}\" packet as an eaglercraft plugin message!", resourcelocation);
            LOGGER.error(e);
        } catch (Exception e) {
    ...
    public EaglercraftUUID func_217277_l() {
