...
public class WorldSelectionList extends ExtendedList<WorldSelectionList.Entry> {
    ...
    public WorldSelectionList(WorldSelectionScreen p_i49846_1_, Minecraft p_i49846_2_, int p_i49846_3_, int p_i49846_4_, int p_i49846_5_, int p_i49846_6_, int p_i49846_7_, Supplier<String> p_i49846_8_, WorldSelectionList p_i49846_9_) {
    ...
    public void setSelected(WorldSelectionList.Entry p_setSelected_1_) {
    ...
    public final class Entry extends ExtendedList.AbstractListEntry<WorldSelectionList.Entry> implements AutoCloseable {
        ...
        public Entry(WorldSelectionList p_i50631_2_, WorldSummary p_i50631_3_, SaveFormat p_i50631_4_) {
            ...
            this.field_214452_e = new ResourceLocation("worlds/" + Integer.toHexString(p_i50631_3_.getFileName().hashCode()) + "/icon");
            this.field_214453_f = p_i50631_4_.getFile(p_i50631_3_.getFileName(), "icon.png");
            if (!this.field_214453_f.exists()) {
        ...
        private DynamicTexture func_214446_f() {
            ...
            if (iconRaw != null) {
                try {
                    int size;
                    if (iconRaw.length == 128 * 128 * 4) {
                        size = 128;
                    } else if (iconRaw.length == 64 * 64 * 4) {
                        size = 64;
                    } else {
                        size = 0;
                    }
                    ...
                    if (size > 0) {
                        ...
                        for (int i = 0; i < pixels.length; ++i) {
                            int j = i << 2;
                            int r = (int) iconRaw[j] & 0xFF;
                            int g = (int) iconRaw[j + 1] & 0xFF;
                            int b = (int) iconRaw[j + 2] & 0xFF;
                            int a = (int) iconRaw[j + 3] & 0xFF;
                            pixels[i] = (a << 24) | (b << 16) | (g << 8) | r;
                        }
                        net.lax1dude.eaglercraft.opengl.ImageData imgData = new net.lax1dude.eaglercraft.opengl.ImageData(size, size, pixels, true);
                        NativeImage nativeimage = new NativeImage(NativeImage.PixelFormat.RGBA, size, size, false);
            ...
            if (this.field_214453_f != null && this.field_214453_f.exists()) {
                try (java.io.InputStream inputstream = this.field_214453_f.getInputStream()) {
                    NativeImage nativeimage = NativeImage.read(inputstream);
                    org.apache.commons.lang3.Validate.validState(nativeimage.getWidth() == 64, "Must be 64 pixels wide");
                    org.apache.commons.lang3.Validate.validState(nativeimage.getHeight() == 64, "Must be 64 pixels high");
                    DynamicTexture dynamictexture = new DynamicTexture(nativeimage);
                    dynamictexture.setBlurMipmap(true, false);
                    this.field_214449_b.getTextureManager().loadTexture(this.field_214452_e, dynamictexture);
                    return dynamictexture;
        ...
        public String getFileName() {
            return this.field_214451_d.getFileName();
        }

        public String getDisplayName() {
            return this.field_214451_d.getDisplayName();
        }
