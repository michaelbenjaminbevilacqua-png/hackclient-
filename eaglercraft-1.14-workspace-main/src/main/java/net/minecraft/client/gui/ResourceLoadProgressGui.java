...
public class ResourceLoadProgressGui extends LoadingGui {
   ...
   static class MojangLogoTexture extends SimpleTexture {
      ...
      protected SimpleTexture.TextureData func_215246_b(IResourceManager resourceManager) {
         try {
            IResource resource = resourceManager.getResource(this.textureLocation);
            if (resource != null) {
               NativeImage nativeImage = NativeImage.read(resource.getInputStream());
               if (nativeImage != null) {
                  return new SimpleTexture.TextureData((TextureMetadataSection)null, nativeImage);
               }
            }
         } catch (Exception e) {
         }
         return new SimpleTexture.TextureData((IOException) new IOException("Failed to load texture"));
      }
   }
}
