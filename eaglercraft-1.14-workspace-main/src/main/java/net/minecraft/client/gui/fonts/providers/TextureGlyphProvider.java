...
public class TextureGlyphProvider implements IGlyphProvider {
    ...
    private final ArrayList<GlyphInfo> glyphInfos;

    public TextureGlyphProvider(NativeImage nativeImageIn, ArrayList<TextureGlyphProvider.GlyphInfo> mapGlyphInfosIn) {
    ...
    public static class Factory implements IGlyphProviderFactory {
        ...
        public IGlyphProvider create(IResourceManager resourceManagerIn) {
            try (IResource iresource = resourceManagerIn.getResource(this.file)) {
                ...
                ArrayList<TextureGlyphProvider.GlyphInfo> char2objectmap = new ArrayList<>();
                ...
                for (int i1 = 0; i1 < this.chars.size(); ++i1) {
                    ...
                    for (int j1 = 0; j1 < s.length(); ++j1) {
                        ...
                        if (c0 != 0 && c0 != ' ') {
                            ...
                            if (char2objectmap.size() <= c0) {
                                char2objectmap.ensureCapacity(c0 + 1);
                                while (char2objectmap.size() <= c0) {
                                    char2objectmap.add(null);
                                }
                            }
                            if (char2objectmap.get(c0) != null) {
                                TextureGlyphProvider.LOGGER.warn("Codepoint '{}' declared multiple times in {}", Integer.toHexString(c0), this.file);
                            } else {
                                char2objectmap.set(c0, new TextureGlyphProvider.GlyphInfo(f, nativeimage, j1 * k, i1 * l, k, l, k1, this.ascent));
                            }
                        }
                    }
                }
    ...
    static final class GlyphInfo implements IGlyphInfo {
        ...
        public float getAdvance() {
            return (float) ((int) (0.5D + (double) ((float) this.advanceWidth * this.field_211582_a)) + 1);
        }
