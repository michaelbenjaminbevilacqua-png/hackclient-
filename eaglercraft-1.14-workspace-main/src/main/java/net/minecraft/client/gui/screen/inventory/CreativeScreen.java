...
public class CreativeScreen extends DisplayEffectsScreen<CreativeScreen.CreativeContainer> {
        ...
        if (slotIn == null && selectedTabIndex != ItemGroup.INVENTORY.getIndex() && type != ClickType.QUICK_CRAFT) {
            ...
            if (slotIn == this.destroyItemSlot && flag) {
                ...
                if (slotIn == this.destroyItemSlot) {
                    ...
                    this.mc.player.container.slotClick(slotIn == null ? slotId : ((CreativeScreen.CreativeSlot) slotIn).slot.slotNumber, mouseButton, type, this.mc.player);
    ...
    private boolean hasTmpInventory(Slot p_208018_1_) {
    ...
    private void updateCreativeSearch() {
        ...
        if (s.isEmpty()) {
            ...
            if (s.startsWith("#")) {
                s = s.substring(1);
                isearchtree = this.mc.func_213253_a(SearchTreeManager.field_215360_b);
                this.searchTags(s);
            } else {
                isearchtree = this.mc.func_213253_a(SearchTreeManager.field_215359_a);
            }
                ...
                if (hotbarsnapshot.isEmpty()) {
                    for (int k = 0; k < 9; ++k) {
                        if (k == j) {
                            ...
                            String s = this.mc.gameSettings.keyBindsHotbar[j].getLocalizedName();
                            String s1 = this.mc.gameSettings.keyBindSaveToolbar.getLocalizedName();
    ...
    protected void renderTooltip(ItemStack p_renderTooltip_1_, int p_renderTooltip_2_, int p_renderTooltip_3_) {
        if (selectedTabIndex == ItemGroup.SEARCH.getIndex()) {
            List<ITextComponent> list = p_renderTooltip_1_.getTooltip(this.mc.player, this.mc.gameSettings.advancedItemTooltips ? ITooltipFlag.TooltipFlags.ADVANCED : ITooltipFlag.TooltipFlags.NORMAL);
    ...
    protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
        ...
        if (itemgroup == ItemGroup.INVENTORY) {
            InventoryScreen.drawEntityOnScreen(this.guiLeft + 88, this.guiTop + 45, 20, (float) (this.guiLeft + 88 - mouseX), (float) (this.guiTop + 45 - 30 - mouseY), this.mc.player);
        }
