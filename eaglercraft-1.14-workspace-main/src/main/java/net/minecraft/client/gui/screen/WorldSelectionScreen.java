...
import net.lax1dude.eaglercraft.sp.gui.GuiScreenBackupWorldSelection;
import net.lax1dude.eaglercraft.sp.gui.GuiScreenCreateWorldSelection;import net.lax1dude.eaglercraft.sp.gui.ScreenLANConnect;
...
public class WorldSelectionScreen extends Screen {
    ...
    protected void init() {
        ...
        this.selectionList = new WorldSelectionList(this, this.mc, this.width, this.height, 48, this.height - 84, 36, () -> {
        ...
        this.backupButton = this.addButton(new Button(this.width / 2 + 4, this.height - 28, 72, 20, I18n.format("selectWorld.edit.backup"), (p_214328_1_) -> {
            this.selectionList.func_214376_a().ifPresent(entry -> {
                this.mc.displayGuiScreen(new GuiScreenBackupWorldSelection(this, entry.getDisplayName(), entry.getFileName()));
            });
        }));
    ...
    @Override
    public boolean mouseClicked(double xx, double yy, int btn) {
        String text = I18n.format("directConnect.lanWorld");
        int w = mc.fontRenderer.getStringWidth(text);
        if (xx > 2 && yy > 2 && xx < (w * 3 / 4) + 5 && yy < 12) {
            if (LANServerController.supported()) {
                mc.displayGuiScreen(new ScreenLANConnect(this));
            } else {
                mc.displayGuiScreen(new ScreenLANNotSupported(this));
            }
        }
        return super.mouseClicked(xx, yy, btn);
    }
