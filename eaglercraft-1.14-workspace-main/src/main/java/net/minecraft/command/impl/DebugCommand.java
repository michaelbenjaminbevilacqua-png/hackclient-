...
public class DebugCommand {
   ...
   private static final java.nio.file.spi.FileSystemProvider field_225391_d = null;
   ...
   private static int stopDebug(CommandSource source) throws CommandSyntaxException {
      ...
      if (!debugprofiler.func_219899_d().isEnabled()) {
         ...
         File file1 = new File(minecraftserver.getFile("debug").getPath(), "profile-results-" + (new SimpleDateFormat("yyyy-MM-dd_HH.mm.ss")).format(new Date()) + ".txt");
         iprofileresult.writeToFile(new net.lax1dude.eaglercraft.internal.vfs2.VFile2(file1.getPath()));
