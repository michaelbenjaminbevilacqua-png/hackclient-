...
public abstract class Entity implements INameable, ICommandSource {
    ...
    private static int clinitToken;
    ...
    protected EaglercraftUUID entityUniqueID = MathHelper.getRandomUUID(this.rand);
    ...
    public void move(MoverType typeIn, Vec3d pos) {
        if (this.noClip) {
            ...
            if (this.motionMultiplier.lengthSquared() > 1.0E-5D) {
            ...
            if (vec3d.lengthSquared() > 1.0E-5D) {
            ...
            this.collidedVertically = !MathHelper.epsilonEquals(pos.y, vec3d.y);
            ...
            if (!MathHelper.epsilonEquals(pos.x, vec3d.x)) {
                this.setMotion(0.0D, vec3d2.y, vec3d2.z);
            }

            if (!MathHelper.epsilonEquals(pos.z, vec3d.z)) {
                this.setMotion(vec3d2.x, vec3d2.y, 0.0D);
            }

            Block block1 = blockstate.getBlock();
            if (!MathHelper.epsilonEquals(pos.y, vec3d.y)) {
    ...
    private Vec3d getAllowedMovement(Vec3d vec) {
        ...
        Stream<VoxelShape> stream = VoxelShapes.compare(voxelshape, VoxelShapes.create(axisalignedbb.shrink(1.0E-5D)), IBooleanFunction.AND) ? Stream.empty() : Stream.of(voxelshape);
        ...
        boolean flag = !MathHelper.epsilonEquals(vec.x, vec3d.x);
        boolean flag1 = !MathHelper.epsilonEquals(vec.y, vec3d.y);
        boolean flag2 = !MathHelper.epsilonEquals(vec.z, vec3d.z);
        ...
        if (this.stepHeight > 0.0F && flag3 && (flag || flag2)) {
            ...
            if (func_213296_b(vec3d1) > func_213296_b(vec3d)) {
                return vec3d1.add(func_223307_a(this, new Vec3d(0.0D, -vec3d1.y + vec.y, 0.0D), axisalignedbb.offset(vec3d1).grow(1.0E-5D, 0.0D, 1.0E-5D), this.world, iselectioncontext, reuseablestream));
            }
        }
    ...
    public static Vec3d func_223307_a(Entity p_223307_0_, Vec3d p_223307_1_, AxisAlignedBB p_223307_2_, World p_223307_3_, ISelectionContext p_223307_4_, ReuseableStream<VoxelShape> p_223307_5_) {
        if (p_223307_1_.lengthSquared() == 0.0D) {
            return p_223307_1_;
        }
    ...
    public boolean isInRangeToRenderDist(double distance) {
        ...
        d0 = d0 * entityRenderMul * renderDistanceWeight;
    ...
    public CompoundNBT writeWithoutTypeId(CompoundNBT compound) {
        try {
            ...
            compound.putUniqueId("EaglercraftUUID", this.getUniqueID());
    ...
    public void read(CompoundNBT compound) {
        try {
            ...
            if (compound.hasUniqueId("EaglercraftUUID")) {
                this.entityUniqueID = compound.getUniqueId("EaglercraftUUID");
    ...
    public String toString() {
        return net.lax1dude.eaglercraft.HString.format("%s['%s'/%d, l='%s', x=%.2f, y=%.2f, z=%.2f]", this.getClass().getSimpleName(), this.getName().getUnformattedComponentText(), this.entityId, this.world == null ? "~NULL~" : this.world.getWorldInfo().getWorldName(), this.posX, this.posY, this.posZ);
    }
    ...
    public void fillCrashReport(CrashReportCategory category) {
        ...
        category.addDetail("Entity's Exact location", net.lax1dude.eaglercraft.HString.format("%.2f, %.2f, %.2f", this.posX, this.posY, this.posZ));
        ...
        Vec3d vec3d = this.getMotion();
        category.addDetail("Entity's Momentum", net.lax1dude.eaglercraft.HString.format("%.2f, %.2f, %.2f", vec3d.x, vec3d.y, vec3d.z));
