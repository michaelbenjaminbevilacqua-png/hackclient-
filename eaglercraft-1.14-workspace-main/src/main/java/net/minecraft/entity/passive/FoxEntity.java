...
public class FoxEntity extends AnimalEntity {
    ...
    private static final DataParameter<Optional<EaglercraftUUID>> TRUSTED_UUID_SECONDARY = EntityDataManager.createKey(FoxEntity.class, DataSerializers.OPTIONAL_UNIQUE_ID);
    private static final DataParameter<Optional<EaglercraftUUID>> TRUSTED_UUID_MAIN = EntityDataManager.createKey(FoxEntity.class, DataSerializers.OPTIONAL_UNIQUE_ID);
    ...
    private List<EaglercraftUUID> getTrustedUUIDs() {
        List<EaglercraftUUID> list = Lists.newArrayList();
        list.add(this.dataManager.get(TRUSTED_UUID_SECONDARY).orElse((EaglercraftUUID) null));
        list.add(this.dataManager.get(TRUSTED_UUID_MAIN).orElse((EaglercraftUUID) null));
        return list;
    }

    private void addTrustedUUID(EaglercraftUUID uuidIn) {
    ...
    public void writeAdditional(CompoundNBT compound) {
        ...
        List<EaglercraftUUID> list = this.getTrustedUUIDs();
        ListNBT listnbt = new ListNBT();

        for (EaglercraftUUID uuid : list) {
            if (uuid != null) {
    ...
    private boolean isTrustedUUID(EaglercraftUUID p_213468_1_) {
    ...
    class RevengeGoal extends NearestAttackableTargetGoal<LivingEntity> {
        ...
        public RevengeGoal(Class<LivingEntity> p_i50743_2_, boolean p_i50743_3_, boolean p_i50743_4_, Predicate<LivingEntity> p_i50743_5_) {
        ...
        public boolean shouldExecute() {
            if (this.targetChance > 0 && this.goalOwner.getRNG().nextInt(this.targetChance) != 0) {
                ...
                for (EaglercraftUUID uuid : FoxEntity.this.getTrustedUUIDs()) {
