 ...
 * order with {@link #firstKey()} without having to create an iterator; however,
...
public class Object2ByteLinkedOpenHashMap<K> extends AbstractObject2ByteSortedMap<K>
    ...
    /*
     * Macros for transforming the bi-directional long link. Return values are
     * 32-bit int indexes. SET_UPPER and SET_LOWER do a masked assignment as
     * described at
     ...
     *
     * <p>
     * If the given entry is the first or the last one, this method will complete in
     * constant time; otherwise, it will have to search for the given entry.
     *
     ...
     * the map
     */
    private final class MapEntry implements Entry<K>, Map.Entry<K, Byte> {
     ...
     *
     * <p>
     * This class provides a list iterator over a linked hash map. The empty
     * constructor runs in constant time. The one-argument constructor needs to
     * search for the given key, but it is optimized for the case of
     * {@link java.util.SortedMap#lastKey()}, in which case runs in constant time,
     * too.
     */
    private class MapIterator {
        ...
        private MapIterator(final K from) {
            if (((key[last]) == null ? (from) == null : (key[last]).equals(from))) {
                ...
                // The starting point.
                int pos = ((from) == null ? 0x87fcd5c : HashCommon.murmurHash3((from).hashCode()))
                        & mask;
                // There's always an unused entry.
                while (used[pos]) {
    ...
    private class EntryIterator extends MapIterator implements ObjectListIterator<Entry<K>> {
        ...
        public void set(Entry<K> ok) {
            throw new UnsupportedOperationException();
        }

        public void add(Entry<K> ok) {
            throw new UnsupportedOperationException();
        }
    }

    private class FastEntryIterator extends MapIterator implements ObjectListIterator<Entry<K>> {
    ...
    private final class MapEntrySet extends AbstractObjectSortedSet<Entry<K>>
            implements FastSortedEntrySet<K> {
        public ObjectBidirectionalIterator<Entry<K>> iterator() {
            return new EntryIterator();
        }

        public Comparator<? super Entry<K>> comparator() {
            return null;
        }

        public ObjectSortedSet<Entry<K>> subSet(Entry<K> fromElement,
                                                Entry<K> toElement) {
            throw new UnsupportedOperationException();
        }

        public ObjectSortedSet<Entry<K>> headSet(Entry<K> toElement) {
            throw new UnsupportedOperationException();
        }

        public ObjectSortedSet<Entry<K>> tailSet(Entry<K> fromElement) {
            throw new UnsupportedOperationException();
        }
        ...
        public ObjectBidirectionalIterator<Entry<K>> iterator(final Entry<K> from) {
            return new EntryIterator((from.getKey()));
        }

        public ObjectBidirectionalIterator<Entry<K>> fastIterator() {
            return new FastEntryIterator();
        }

        public ObjectBidirectionalIterator<Entry<K>> fastIterator(final Entry<K> from) {
    ...
    private final class KeySet extends AbstractObjectSortedSet<K> {
        ...
        final public ObjectSortedSet<K> tailSet(K from) {
            throw new UnsupportedOperationException();
        }

        final public ObjectSortedSet<K> headSet(K to) {
            throw new UnsupportedOperationException();
        }

        final public ObjectSortedSet<K> subSet(K from, K to) {
            throw new UnsupportedOperationException();
        }
    }
    ...
    private final class ValueIterator extends MapIterator implements ByteListIterator {
        ...
        public Byte previous() {
            return (Byte.valueOf(value[previousEntry()]));
        }
