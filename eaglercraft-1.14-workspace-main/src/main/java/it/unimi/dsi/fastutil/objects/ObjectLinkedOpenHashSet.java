 ...
 * iteration order with {@link #first()} without having to create an iterator;
...
public class ObjectLinkedOpenHashSet<K> extends AbstractObjectSortedSet<K>
     ...
     * @throws IllegalArgumentException if <code>from</code> does not belong to the
     *                                  set.
     */
     ...
     *
     * <p>
     * This class provides a list iterator over a linked hash set. The empty
     * constructor runs in constant time. The one-argument constructor needs to
     * search for the given element, but it is optimized for the case of
     * {@link java.util.SortedSet#last()}, in which case runs in constant time, too.
     */
    private class SetIterator extends AbstractObjectListIterator<K> {
        ...
        public K next() {
            ...
            if (index >= 0)
                index++;
            if (ASSERTS)
                assert used[curr] : "Position " + curr + " is not used";
            return key[curr];
        }
