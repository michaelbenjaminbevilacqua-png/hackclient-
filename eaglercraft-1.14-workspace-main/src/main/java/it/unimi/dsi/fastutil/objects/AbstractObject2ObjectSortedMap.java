...
public abstract class AbstractObject2ObjectSortedMap<K, V> extends AbstractObject2ObjectMap<K, V>
    ...
    protected class KeySet extends AbstractObjectSortedSet<K> {
        ...
        public ObjectBidirectionalIterator<K> iterator(final K from) {
            return new KeySetIterator<K, V>(entrySet().iterator(new BasicEntry<K, V>(from, (null))));
        }

        public ObjectBidirectionalIterator<K> iterator() {
            return new KeySetIterator<K, V>(entrySet().iterator());
        }
    }
    ...
    protected class ValuesCollection extends AbstractObjectCollection<V> {
        public ObjectIterator<V> iterator() {
            return new ValuesIterator<K, V>(entrySet().iterator());
        }
