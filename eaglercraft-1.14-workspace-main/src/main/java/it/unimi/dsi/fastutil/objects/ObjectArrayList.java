...
public class ObjectArrayList<K> extends AbstractObjectList<K> implements RandomAccess, Cloneable, java.io.Serializable {
    ...
    /**
     * Whether the backing array was passed to <code>wrap()</code>. In this case, we
     * must reallocate with the same type of array.
     */
     ...
     * errors if a method expects an array actually of type <code>K[]</code>, but
     * this methods returns an array of type {@link Object Object[]}.
     *
    ...
    public void size(final int size) {
        if (size > a.length)
            ensureCapacity(size);
        if (size > this.size)
            ObjectArrays.fill(a, this.size, size, (null));
        else
            ObjectArrays.fill(a, size, this.size, (null));
        this.size = size;
    }
