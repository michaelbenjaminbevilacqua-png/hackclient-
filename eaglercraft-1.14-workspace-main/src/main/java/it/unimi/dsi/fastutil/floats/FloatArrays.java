...
public class FloatArrays {
     ...
     * handle <code>null</code> correctly, and it is serializable.
     */
    public final static Hash.Strategy<float[]> HASH_STRATEGY = new ArrayHashStrategy();
     ...
     * should probably use <code>grow()</code> instead.
     *
     ...
     * whose first <code>array.length</code> entries are the same as those
     * of <code>array</code>.
     */
     ...
     * entries whose entries are the same as the first <code>length</code>
     * entries of <code>array</code>; otherwise, an array with
     * <code>length</code> entries whose first <code>array.length</code>
     ...
     *
     * <p>
     * If possible (i.e., <code>from</code> is 0) this method uses a backward loop.
     * In this case, it is significantly faster than the corresponding method in
     ...
     *                                        <code>to</code> are greater than the
    ...
    /**
     * Sorts the specified range of elements according to the natural ascending
     * order using mergesort, using a given support array.
     *
     * <p>
     ...
     * <p>
     * This sort is guaranteed to be <i>stable</i>: equal elements will not be
     * reordered as a result of the sort. An array as large as <code>a</code> will
    ...
    /**
     * Sorts the specified range of elements according to the order induced by the
     * specified comparator using mergesort, using a given support array.
     *
     * <p>
     * This sort is guaranteed to be <i>stable</i>: equal elements will not be
     * reordered as a result of the sort. Moreover, no support arrays will be
     ...
     * <samp>(-(<i>insertion point</i>) - 1)</samp>. The <i>insertion
     ...
     * the key, or the length of the array, if all elements in the array are
    ...
    public static void radixSort(final float[] a, final int from, final int to) {
        ...
        while (offsetPos > 0) {
            ...
            for (int i = length; i-- != 0; )
                digit[i] = (byte) (((fixFloat(a[first + i]) >>> shift) & DIGIT_MASK) ^ signMask);
            for (int i = length; i-- != 0; )
                count[digit[i] & 0xFF]++;
     ...
     *
     * <p>
     * This implementation is significantly faster than quicksort (unstable) or
     * mergesort (stable) already at small sizes (say, more than 10000 elements),
     * but it can only sort in ascending order. It will allocate a support array of
     * bytes with the same number of elements as the array to be sorted, and, in the
     * stable case, a further support array as large as <code>perm</code> (note that
     * the stable version is slightly faster).
     *
     ...
     * @param perm   a permutation array indexing <code>a</code>.
     * @param a      the array to be sorted.
     * @param from   the index of the first element of <code>perm</code> (inclusive)
     *               to be permuted.
     * @param to     the index of the last element of <code>perm</code> (exclusive)
     *               to be permuted.
     * @param stable whether the sorting algorithm should be stable.
     */
    public static void radixSortIndirect(final int[] perm, final float[] a, final int from, final int to,
        ...
        while (offsetPos > 0) {
            ...
            // Count keys.
            for (int i = length; i-- != 0; )
                digit[i] = (byte) (((fixFloat(a[perm[first + i]]) >>> shift) & DIGIT_MASK) ^ signMask);
            for (int i = length; i-- != 0; )
                count[digit[i] & 0xFF]++;
            // Compute cumulative distribution and push non-singleton keys on stack.
            int lastUsed = -1;
     ...
     * <code>a[ i ] < a[ i + 1 ]</code> or <code>a[ i ] == a[ i + 1 ]</code> and
     * <code>b[ i ] <= b[ i + 1 ]</code>.
     *
     * <p>
     * This implementation is significantly faster than quicksort already at small
    ...
    public static void radixSort(final float[] a, final float[] b, final int from, final int to) {
        ...
        while (offsetPos > 0) {
            ...
            // Count keys.
            for (int i = length; i-- != 0; )
                digit[i] = (byte) (((fixFloat(k[first + i]) >>> shift) & DIGIT_MASK) ^ signMask);
            for (int i = length; i-- != 0; )
                count[digit[i] & 0xFF]++;
            // Compute cumulative distribution and push non-singleton keys on stack.
            int lastUsed = -1;
    ...
    public static void radixSortIndirect(final int[] perm, final float[] a, final float[] b, final int from,
        ...
        while (offsetPos > 0) {
            ...
            // Count keys.
            for (int i = length; i-- != 0; )
                digit[i] = (byte) (((fixFloat(k[perm[first + i]]) >>> shift) & DIGIT_MASK) ^ signMask);
            for (int i = length; i-- != 0; )
                count[digit[i] & 0xFF]++;
            // Compute cumulative distribution and push non-singleton keys on stack.
            int lastUsed = -1;
     ...
     * @param random a pseudorandom number generator (please use a <a href=
     *               "http://dsiutils.dsi.unimi.it/docs/it/unimi/dsi/util/XorShiftStarRandom.html">XorShift*</a>
     *               generator).
     * @return <code>a</code>.
     */
