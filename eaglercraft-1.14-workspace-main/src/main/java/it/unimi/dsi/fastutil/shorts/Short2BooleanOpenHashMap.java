...
public class Short2BooleanOpenHashMap extends AbstractShort2BooleanMap
    ...
    public Boolean get(final Short ok) {
        ...
        while (used[pos]) {
            if (((key[pos]) == (k)))
                return (Boolean.valueOf(value[pos]));
            pos = (pos + 1) & mask;
        }
        return (null);
    }
     ...
     * the map
     */
    private final class MapEntry implements Entry, Map.Entry<Short, Boolean> {
    ...
    private final class KeyIterator extends MapIterator implements ShortIterator {
        ...
        public Short next() {
            return (Short.valueOf(key[nextEntry()]));
        }
    }
