 ...
 * {@linkplain Strategy hashing strategy} is
...
public class Object2FloatOpenCustomHashMap<K> extends AbstractObject2FloatMap<K>
    ...
    public float put(final K k, final float v) {
        // The starting point.
        int pos = (HashCommon.murmurHash3(strategy.hashCode((K) (k)))) & mask;
    ...
    protected final int shiftKeys(int pos) {
        ...
        for (; ; ) {
            pos = ((last = pos) + 1) & mask;
            while (used[pos]) {
                slot = (HashCommon.murmurHash3(strategy.hashCode((K) (key[pos])))) & mask;
    ...
    protected void rehash(final int newN) {
        ...
        for (int j = size; j-- != 0; ) {
            while (!used[i])
                i++;
            k = key[i];
            pos = (HashCommon.murmurHash3(strategy.hashCode((K) (k)))) & newMask;
            while (newUsed[pos])
                pos = (pos + 1) & newMask;
            newUsed[pos] = true;
            newKey[pos] = k;
    ...
    private void readObject(java.io.ObjectInputStream s) throws java.io.IOException, ClassNotFoundException {
        ...
        for (int i = size, pos = 0; i-- != 0; ) {
            k = (K) s.readObject();
            v = s.readFloat();
            pos = (HashCommon.murmurHash3(strategy.hashCode((K) (k)))) & mask;
            while (used[pos])
                pos = (pos + 1) & mask;
            used[pos] = true;
            key[pos] = k;
    ...
    private final class MapEntry implements Entry<K>, Map.Entry<K, Float> {
        ...
        public boolean equals(final Object o) {
            ...
            return (strategy.equals((key[index]), (K) ((e.getKey()))))
                    && ((value[index]) == (((e.getValue()).floatValue())));
        }

        public int hashCode() {
            return (strategy.hashCode((K) (key[index]))) ^ HashCommon.float2int(value[index]);
        }
