...
public class Long2ByteOpenHashMap extends AbstractLong2ByteMap implements java.io.Serializable, Cloneable, Hash {
    ...
    protected final int shiftKeys(int pos) {
        ...
        for (; ; ) {
            pos = ((last = pos) + 1) & mask;
            while (used[pos]) {
                slot = (int) HashCommon.murmurHash3(key[pos]) & mask;
    ...
    protected void rehash(final int newN) {
        ...
        for (int j = size; j-- != 0; ) {
            while (!used[i])
                i++;
            k = key[i];
            pos = (int) HashCommon.murmurHash3(k) & newMask;
            while (newUsed[pos])
                pos = (pos + 1) & newMask;
            newUsed[pos] = true;
            newKey[pos] = k;
    ...
    private void readObject(java.io.ObjectInputStream s) throws java.io.IOException, ClassNotFoundException {
        ...
        for (int i = size, pos = 0; i-- != 0; ) {
            k = s.readLong();
            v = s.readByte();
            pos = (int) HashCommon.murmurHash3(k) & mask;
            while (used[pos])
                pos = (pos + 1) & mask;
            used[pos] = true;
            key[pos] = k;
     ...
     * the map
     */
    private final class MapEntry implements Entry, Map.Entry<Long, Byte> {
        ...
        public int hashCode() {
            return HashCommon.long2int(key[index]) ^ (value[index]);
        }
    ...
    private class FastEntryIterator extends MapIterator implements ObjectIterator<Entry> {
        final BasicEntry entry = new BasicEntry(((long) 0), ((byte) 0));
    ...
    private final class MapEntrySet extends AbstractObjectSet<Entry> implements FastEntrySet {
        ...
        public boolean contains(final Object o) {
            ...
            while (used[pos]) {
                if (((key[pos]) == (k)))
                    return ((value[pos]) == (((e.getValue()).byteValue())));
                pos = (pos + 1) & mask;
            }
            return false;
        }
