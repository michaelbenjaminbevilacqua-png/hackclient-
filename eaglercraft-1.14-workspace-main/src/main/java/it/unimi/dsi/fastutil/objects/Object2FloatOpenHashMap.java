 ...
 * issue in the section &ldquo;Faster Hash Tables&rdquo; of the
...
public class Object2FloatOpenHashMap<K> extends AbstractObject2FloatMap<K>
    ...
    public float put(final K k, final float v) {
        // The starting point.
        int pos = ((k) == null ? 0x87fcd5c : HashCommon.murmurHash3((k).hashCode())) & mask;
     ...
     * @deprecated use <code>addTo()</code> instead; having the same name of a
     * {@link java.util.Set} method turned out to be a recipe for
     * disaster.
     */
    @Deprecated
    ...
    protected final int shiftKeys(int pos) {
        ...
        for (; ; ) {
            pos = ((last = pos) + 1) & mask;
            while (used[pos]) {
                slot = ((key[pos]) == null ? 0x87fcd5c
                        : HashCommon.murmurHash3((key[pos]).hashCode())) & mask;
    ...
    protected void rehash(final int newN) {
        ...
        for (int j = size; j-- != 0; ) {
            while (!used[i])
                i++;
            k = key[i];
            pos = ((k) == null ? 0x87fcd5c : HashCommon.murmurHash3((k).hashCode())) & newMask;
            while (newUsed[pos])
                pos = (pos + 1) & newMask;
            newUsed[pos] = true;
            newKey[pos] = k;
    ...
    public int hashCode() {
        ...
        for (int j = size, i = 0, t = 0; j-- != 0; ) {
            ...
            t ^= HashCommon.float2int(value[i]);
            h += t;
            i++;
        }
        return h;
    ...
    private void readObject(java.io.ObjectInputStream s) throws java.io.IOException, ClassNotFoundException {
        ...
        for (int i = size, pos = 0; i-- != 0; ) {
            k = (K) s.readObject();
            v = s.readFloat();
            pos = ((k) == null ? 0x87fcd5c : HashCommon.murmurHash3((k).hashCode())) & mask;
            while (used[pos])
                pos = (pos + 1) & mask;
            used[pos] = true;
            key[pos] = k;
     ...
     * the map
     */
    private final class MapEntry implements Entry<K>, Map.Entry<K, Float> {
        ...
        public boolean equals(final Object o) {
            ...
            return ((key[index]) == null ? ((e.getKey())) == null : (key[index]).equals((e.getKey())))
                    && ((value[index]) == (((e.getValue()).floatValue())));
        }

        public int hashCode() {
            return ((key[index]) == null ? 0 : (key[index]).hashCode())
                    ^ HashCommon.float2int(value[index]);
        }
    ...
    private class MapIterator {
        /**
         * The index of the next entry to be returned, if positive or zero. If negative,
         * the next entry to be returned, if any, is that of index -pos -2 from the
         * {@link #wrapped} list.
         */
        int pos = Object2FloatOpenHashMap.this.n;
        /**
         * The index of the last entry that has been returned. It is -1 if either we did
         * not return an entry yet, or the last returned entry has been removed.
         */
        int last = -1;
        /**
         ...
         * around the table because of removals; such elements would not be enumerated
        ...
        public int nextEntry() {
            ...
            if (pos < 0) {
                ...
                // The starting point.
                int pos = ((k) == null ? 0x87fcd5c : HashCommon.murmurHash3((k).hashCode()))
                        & mask;
                // There's always an unused entry.
                while (used[pos]) {
                    if (((key[pos]) == null ? (k) == null : (key[pos]).equals(k)))
            ...
            if (c != 0) {
                ...
                while (pos-- != 0 && !used[pos])
                    ;
                // When here pos < 0 there are no more elements to be enumerated by scanning,
                // but wrapped might be nonempty.
            }
            return last;
        }
        ...
        /**
         * Shifts left entries with the specified hash code, starting at the specified
         * position, and empties the resulting free entry. If any entry wraps around the
         * table, instantiates lazily {@link #wrapped} and stores the entry key.
         *
         * @param pos a starting position.
         * @return the position cleared by the shifting process.
         */
    ...
    private class EntryIterator extends MapIterator implements ObjectIterator<Entry<K>> {
    ...
    private class FastEntryIterator extends MapIterator implements ObjectIterator<Entry<K>> {
    ...
    private final class MapEntrySet extends AbstractObjectSet<Entry<K>> implements FastEntrySet<K> {
        ...
        public boolean contains(final Object o) {
            ...
            while (used[pos]) {
                if (((key[pos]) == null ? (k) == null : (key[pos]).equals(k)))
                    return ((value[pos]) == (((e.getValue()).floatValue())));
                pos = (pos + 1) & mask;
            }
            return false;
        }
